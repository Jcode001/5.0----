<template>
    <div style="margin-left:1.1vw;">
        <div  v-show="isCustomOptionSet_DA" :class="isCustomOptionSet_DA ? 'newisCollapse' : ''">
          <div style="width:12vw;">
            ...
            <div style="margin-top:.5vw;">
              <el-tree  ref="tree" highlight-current accordion :class="newclassone ? 'newclassDa' : 'oldclassDa'" style="width:11.4vw;"
                :data="treeData" :current-node-key="currentNodeKey" node-key="id"
      ...
      <el-dialog center title="（档案）自定义分组设置" :visible.sync="dialogVisiblesetting" width="80%" @close="dialogClose()">
        <div>
          <div>
            <div class="box_cm">
...
export default {
    ...
    props: ["tableName","isCustomOptionSet_DA","MlId","newclassone"],
    data() {
      return {
    ...
    created () {
      console.log('进入到(档案)自定义分组设置页面');
      console.log(this.tableName,this.isCustomOptionSet_DA,this.MlId,this.newclassone,'tableName,isCustomOptionSet_DA,MlId newclassone');
      if(this.tableName==''){
        this.dialogVisiblesetting=false;
        this.$message.error("请选择档案分类");
        return;
      }
      if(!this.isCustomOptionSet_DA){
        this.dialogVisiblesetting=false;
        return;
      }
      this.initSelected();
    ...
    methods: {
      init(){
        ...
        axios.post("http://" + this.ServeIp + ":" + this.ServePort + "/GetZLXData", getAllTableData).then(//（盒）获取所有著录项列表
          (success) => {
      ...
      initSelected(){
        ...
        axios.post("http://" + this.ServeIp + ":" + this.ServePort + "/GetZDYGroupData", getTableData).then(//获取（盒）已选著录项列表
          (success) => {
      ...
      getZDYGroupNode(){
        ...
        console.log(this.selectedTableData,'selectedTableDataselectedTableDataselectedTableDataselectedTableData');
        ...
        axios.post("http://" + this.ServeIp + ":" + this.ServePort + "/GetZDYGroupNode", getZDYGroupNodeData).then(//（盒）获取自定义项的树型结构
          (success) => {
      ...
      Right() {//左到右
        ...
        selectList.forEach((item) => {
          ...
          if (index !== undefined) {
            ...
            axios.post("http://" + this.ServeIp + ":" + this.ServePort + "/AddZDYGroupData", rightData).then(//添加（盒）已选著录项列表
              (success) => {
      ...
      Left() {//右到左
        ...
        selectList.forEach((item) => {
          ...
          if (index !== undefined) {
            ...
            axios.post("http://" + this.ServeIp + ":" + this.ServePort + "/DelZDYGroupData", rightData).then(//删除（盒）已选著录项列表 
              (success) => {
      ...
      moveUp(index,row) {//上移
        ...
        axios.post("http://" + this.ServeIp + ":" + this.ServePort + "/updateZDYGroupxsort1", updateZDYGroupData).then(//上移
          (res) => {
      ...
      moveDown(index,row) {//下移
        ...
        axios.post("http://" + this.ServeIp + ":" + this.ServePort + "/updateZDYGroupxsort1", updateZDYGroupData).then(//下下下移
          (res) => {
      ...
      handleNodeClick(data,node,object) {
        ...
        if(node.level==1){//第一一一一一一一一一一一级
          ...
          axios.post("http://" + this.ServeIp + ":" + this.ServePort + "/GetZDYGroupNode", datas).then(//（盒）获取(二级)自定义项的树型结构
            (success) => {
              var res=success.data.data;
          ...
          axios.post("http://" + this.ServeIp + ":" + this.ServePort + "/GetZDYGroupNode", datas).then(//（盒）获取(三级)自定义项的树型结构
            (success) => {
              var res=success.data.data;
          ...
          var datas={
            ...
            axios.post("http://" + this.ServeIp + ":" + this.ServePort + "/GetZDYGroupNode", datas).then(//（盒）获取(五级)自定义项的树型结构
              (success) => {
                console.log(success,'success');
                var res=success.data.data;
                data.children=[];
          ...
          var datas={
            ...
            axios.post("http://" + this.ServeIp + ":" + this.ServePort + "/GetZDYGroupNode", datas).then(//（盒）获取(六级)自定义项的树型结构
              (success) => {
                console.log(success,'success');
                var res=success.data.data;
                data.children=[];
