...
export function asdbasd() {
  setTimeout(() => {
    ...
    console.log(WebVideoCtrl, 'WebVideoCtrlWebVideoCtrl');
...
// 登录
export function clickLogin(ip, port, user, password, Channel,CameraId) {
  let steptimelen=1000;
  let logokflag=false;
  ...
  console.log("第1步:",ip, port, user, password, Channel, '进入登录接口ip, port, user, password')
  ...
  // setTimeout(function () {
  //   if(logokflag){
  //       console.log("第4步:获取通道！");
  //       getChannelInfo(szIP, szChannel);
  //   }
  // }, 5*steptimelen);
  setTimeout(function () {
    console.log("第4步:停止预览123123123！");
    if(logokflag){
        console.log("第4步:停止预览！");
        clickStopRealPlay(szDeviceIdentify,szChannel);
    }
  }, 6*steptimelen);
  ...
  setTimeout(function () {
    if(logokflag){
        console.log("第5步:播放！");
        clickStartRealPlay(szDeviceIdentify,szChannel);
    }
  }, 9*steptimelen);
...
// 退出
export function clickLogout(ip) {
...
// // 获取设备信息
// export function clickGetDeviceInfo(ip) {
...
// 获取通道
// export function getChannelInfo(szIP, szChannel) {
...
// 获取端口
// export function getDevicePort(ip) {
//   var szDeviceIdentify = $("#ip").val();
...
// }
// 开始预览
export function clickStartRealPlay(szDeviceIdentify,szChannel) {
  var iStreamType = 1;
  console.log(szDeviceIdentify,'szDeviceIdentifyszDeviceIdentifyszDeviceIdentify');
  console.log(g_iWndIndex, '第一个——————————————————————————————————————开始预览窗口编号');
  console.log(iStreamType, 'iStreamType开始预览');
  console.log(parseInt(szChannel, 10), 'iChannelID');
  ...
  var startRealPlay = function () {
    console.log(szDeviceIdentify, g_iWndIndex, iStreamType, szChannel, bZeroChannel, ' 开始预览需要 的数据');
    console.log('192.168.2.169_80', 0, 1, 12, false, '开始预览需要的数据11111');
