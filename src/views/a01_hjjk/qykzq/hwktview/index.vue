		...
		　  looptimer = setInterval("showAuto()", 1000*60);//界面刷新时间30S
			 ...
			 $("#btnopen2").bind("click",function(){
		    	 ...
		    	 var indata={
			            	...
			            	"port":"8066",
							      "method":"KFPTService/setPower",
							     "jsonin":"{\"ip\":\""+$('#HostName').html()+"\",\"deviceType\":5,\"deviceNo\":\""+$("#datadeviceNo").html()+"\",\"operaFlag\":2}"
			            }
			     $.ajax({
					type: "GET",
					url: "http://127.0.0.1:4444/getRfid",
			...
			$("#btnTempSet").bind("click",function(){
		    	 var indata={
			            	...
			            	"port":"8066",
							      ...
							      "jsonin":"{\"ip\":\""+$('#HostName').html()+"\",\"deviceType\":5,\"deviceNo\":\""+$("#datadeviceNo").html()+"\",\"minTemp\":"+$("#minTemp").val()+",\"maxTemp\":"+$("#maxTemp").val()+"}"
			            }
			     $.ajax({
					type: "GET",
					url: "http://127.0.0.1:4444/getRfid",
		...
		function getdata(indata,ctlname)
		{
			$.ajax({
					...
					success: function(Obj,status){
						...
						$("#datatemp").html("<i>"+obj1.data5[DeviceID].datatemp+"<small>°C</small></i>");
						$("#datahum").html("<i>"+obj1.data5[DeviceID].datahum+"<small>%RH</small></i>");
						
						$("#minTemp").val(obj1.data5[DeviceID].dataminTemp);
						$("#maxTemp").val(obj1.data5[DeviceID].datamaxTemp);
						
						//$("#dataDLMX").val(obj1.data5[DeviceID].dataDLMX);
						var airStatuslist=new Array("关","制冷","制热");
						$("#airStatus").html(airStatuslist[obj1.data5[DeviceID].dataairStatus]);
...
<div style="position:absolute;right:2%;top:0%;width:80%;height:100%;background-color:#e5eef7;border-radius:12px">
  ...
  <span style="position:absolute;left:15.4%;top:26%;font-size:50px;color:#ffffff" id="datatemp"></span>
  <span style="position:absolute;left:44.5%;top:26%;font-size:52px;color:#ffffff" id="datahum"></span>
  <div style="position:absolute;left:7%;top:49%;width:60%;height:40px;border:0px solid red;font-size:20px;line-height:40px">
  手动控制&emsp;
  <button id="btnclose" style="width:10%;height:100%;border:3px solid #036ff6;color:#ffffff;background-color:#036ff6;border-radius:20px;font-size:18px;outline:medium">关机</button>
  <button id="btnopen" style="width:10%;height:100%;border:3px solid #036ff6;color:#036ff6;background-color:#ffffff;border-radius:20px;font-size:18px;outline:medium">制冷</button>
  <button id="btnopen2" style="width:10%;height:100%;border:3px solid #036ff6;color:#036ff6;background-color:#ffffff;border-radius:20px;font-size:18px;outline:medium">制热</button>
  &emsp;&emsp;&emsp;
  模&emsp;式&emsp;
  ...
  <div style="position:absolute;left:7%;top:70%;width:60%;height:40px;border:0px solid red;font-size:20px;line-height:40px;">
   ...
   <select id="DeviceType" style="position:absolute;left:2.3%;top:3.8%;width:15%;height:40px;border:1px solid #e6eef8;border-radius:6px;outline:none">
	...
	<option value="4" >酸性气体空气净化机</option>
  	<option value="5" selected>红外空调</option>
