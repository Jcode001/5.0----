...
            $("#sysname").html(sysname)
		...
		　  looptimer= setInterval("showAuto()",1000*60); 
			...
			$("#btnhsdopen").bind("click",function(){
			     clearInterval(looptimer);
				 looptimer= setInterval("showAuto()",1000*60);
			...
			$("#btnhsdclose").bind("click",function(){
			     clearInterval(looptimer);
				 looptimer= setInterval("showAuto()",1000*60);
		...
		function getdata(indata,ctlname)
		{
			$.ajax({
					...
					success: function(Obj,status){
						...
						$("#pm2_5").html("<i>"+obj1.data2[DeviceID].datapm2_5+"<small>ug/m³</small></i>");
						$("#datahum").html("<i>"+obj1.data2[DeviceID].datahum+"<small>%RH</small></i>");
						$("#CO2").html("<i>"+obj1.data2[DeviceID].dataCO2+"<small>ppm</small></i>");
						
						fsstate=obj1.data2[DeviceID].dataFJZT;
...
<div style="position:absolute;right:2%;top:0%;width:80%;height:100%;background-color:#e5eef7;border-radius:12px">
  ...
  <span style="position:absolute;left:31%;top:31%;font-size:28px;color:#036ff6" id="pm2_5"></span>
  ...
  <div style="position:absolute;left:7%;top:76%;width:60%;height:40px;border:0px solid red;font-size:20px;line-height:40px">
  恒时灯&emsp;
  <button id="btnhsdopen" style="width:10%;height:100%;border:3px solid #036ff6;color:#ffffff;background-color:#036ff6;border-radius:20px;font-size:18px;outline:medium"">开</button>
  <button id="btnhsdclose" style="width:10%;height:100%;border:3px solid #036ff6;color:#036ff6;background-color:#ffffff;border-radius:20px;font-size:18px;outline:medium"">关</button>
  &emsp;&emsp;&emsp;&emsp;
  风&emsp;速&emsp;
  ...
  <button id="fsstate3" style="width:5%;height:100%;border:3px solid #036ff6;color:#036ff6;background-color:#ffffff;font-size:18px;border-radius:0 20px 20px 0;outline:medium">高</button>
   ...
   <select id="DeviceType" style="position:absolute;left:2.3%;top:3.8%;width:15%;height:40px;border:1px solid #e6eef8;border-radius:6px;outline:none">
	...
	<option value="1" >净化除酸型除湿加湿一体机</option>
  	<option value="2" selected>健康防护一体机</option>
