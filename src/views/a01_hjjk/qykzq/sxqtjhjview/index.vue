		...
		　  looptimer= setInterval("showAuto()",1000*60); 
		    $("#btnopen").bind("click",function(){
		    	 ...
		    	 var indata={
			            	...
			            	"port":"8066",
							     ...
							     "jsonin":"{\"ip\":\""+$('#HostName').html()+"\",\"deviceType\":4,\"deviceNo\":\""+$("#datadeviceNo").html()+"\",\"operaFlag\":1}"
			            }
			     $.ajax({
					type: "GET",
					url: "http://127.0.0.1:4444/getRfid",
			...
			$("#btnclose").bind("click",function(){
		    	 ...
		    	 var indata={
			            	...
			            	"port":"8066",
							      "method":"KFPTService/setPower",
							      "jsonin":"{\"ip\":\""+$('#HostName').html()+"\",\"deviceType\":4,\"deviceNo\":\""+$("#datadeviceNo").html()+"\",\"operaFlag\":0}"
			            }
			     $.ajax({
					type: "GET",
					url: "http://127.0.0.1:4444/getRfid",
			...
			$("#btnzropen").bind("click",function(){
			     clearInterval(looptimer);
				 looptimer= setInterval("showAuto()",1000*60);
			     $("#btnzropen").css("color","#ffffff");
				 $("#btnzropen").css("background-color","#036ff6");
				 $("#btnzrclose").css("color","#036ff6");
				 $("#btnzrclose").css("background-color","#ffffff");
		    	 var indata={
			            	"cmd":60001,
			            	"localip":"127.0.0.1",
			            	"hostname":"10.168.1.39",
			            	"port":"8066",
							      "method":"KFPTService/setCSXKQJHJZR",
							     "jsonin":"{\"ip\":\""+$('#HostName').html()+"\",\"deviceType\":4,\"deviceNo\":\""+$("#datadeviceNo").html()+"\",\"operaFlag\":1}"
			            }
			     $.ajax({
					type: "GET",
			...
			$("#btnzrclose").bind("click",function(){
			     clearInterval(looptimer);
				 looptimer= setInterval("showAuto()",1000*60);
			     $("#btnzrclose").css("color","#ffffff");
				 $("#btnzrclose").css("background-color","#036ff6");
				 $("#btnzropen").css("color","#036ff6");
				 $("#btnzropen").css("background-color","#ffffff");
		    	 var indata={
			            	"cmd":60001,
			            	"localip":"127.0.0.1",
			            	"hostname":"10.168.1.39",
			...
			$("#fsstate0").bind("click",function(){
		    	 ...
		    	 var indata={
			            	...
			            	"port":"8066",
							     ...
							     "jsonin":"{\"ip\":\""+$('#HostName').html()+"\",\"deviceType\":4,\"deviceNo\":\""+$("#datadeviceNo").html()+"\",\"operaFlag\":"+operaFlag+"}"
			            }
			     $.ajax({
					type: "GET",
					url: "http://127.0.0.1:4444/getRfid",
		...
		function getdata(indata,ctlname)
		{
			$.ajax({
					...
					success: function(Obj,status){
						...
						console.log(JSON.stringify(obj1.data4[DeviceID]))
						$("#datadeviceNo").html(obj1.data4[DeviceID].datadeviceNo);
						$("#dataTXZT").html(obj1.data4[DeviceID].dataTXZT==1?"离线":"在线");
						$("#datatemp").html("<i>"+obj1.data4[DeviceID].datatemp+"<small>°C</small></i>");
						$("#pm2_5").html("<i>"+obj1.data4[DeviceID].datapm2_5+"<small>ug/m³</small></i>");
						$("#datahum").html("<i>"+obj1.data4[DeviceID].datahum+"<small>%RH</small></i>");
						$("#TVOC").html("<i>"+obj1.data4[DeviceID].dataTVOC+"<small>mg/m³</small></i>");
						$("#SO2").html("<i>"+obj1.data4[DeviceID].dataSO2+"<small>ug/m³</small></i>");
						$("#HCHO").html("<i>"+obj1.data4[DeviceID].dataHCHO+"<small>mg/m³</small></i>");
...
<div style="position:absolute;right:2%;top:0%;width:80%;height:100%;background-color:#e5eef7;border-radius:12px">
  ...
  <span style="position:absolute;left:12.5%;top:32%;font-size:26px;color:#036ff6" id="datatemp"></span>
  <span style="position:absolute;left:31%;top:32%;font-size:26px;color:#036ff6" id="pm2_5"></span>
  <span style="position:absolute;left:52%;top:32%;font-size:26px;color:#036ff6" id="datahum"></span>
  <span style="position:absolute;left:21%;top:47%;font-size:16px;color:#51de90" id="TVOC"></span>
  <span style="position:absolute;left:53%;top:47%;font-size:16px;color:#2898ca" id="SO2"></span>
  <span style="position:absolute;left:21%;top:58%;font-size:16px;color:#18d7cb" id="HCHO"></span>
  ...
  &emsp;&emsp;
  制&emsp;热
  <button id="btnzropen" style="width:10%;height:100%;border:3px solid #036ff6;color:#ffffff;background-color:#036ff6;border-radius:20px;font-size:18px;outline:medium">开</button>
  <button id="btnzrclose" style="width:10%;height:100%;border:3px solid #036ff6;color:#036ff6;background-color:#ffffff;border-radius:20px;font-size:18px;outline:medium">关</button>
  </div>
  ...
  <button id="fsstate3" style="width:5%;height:100%;border:3px solid #036ff6;color:#036ff6;background-color:#ffffff;font-size:18px;border-radius:0 20px 20px 0;outline:medium">高</button>
   ...
   <select id="DeviceType" style="position:absolute;left:2.3%;top:3.8%;width:15%;height:40px;border:1px solid #e6eef8;border-radius:6px;outline:none">
	...
	<option value="4" selected>酸性气体空气净化机</option>
  ...
  <select id="DeviceID" style="position:absolute;right:5%;top:20%;width:25%;height:40px;border:1px solid #e6eef8;border-radius:6px;outline:none">
  	<option value="0" selected>1号酸性气体空气净化机</option>
  	<option value="1">2号酸性气体空气净化机</option>
  	<option value="2">3号酸性气体空气净化机</option>
  	<option value="3">4号酸性气体空气净化机</option>
  	<option value="4">5号酸性气体空气净化机</option>
  </select>
</div></body>
</html>