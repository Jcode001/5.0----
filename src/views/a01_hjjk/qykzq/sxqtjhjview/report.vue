...
            $("#sysname").html(sysname)
		...
		function showAuto(){
				...
				indata['jsonin']='{"ip":"'+$('#HostName').html()+'","deviceType":"4","deviceNo":"'+$('#DeviceID').val()+'","startDate": "'+startdate+'","endDate": "'+enddate+'"}';
				//console.log(indata)
						...
						success: function(Obj,status){
							...
							if(Obj.flag==1)
							{
								...
								for(i=0;i<datas.length;i++)
								{
									...
									$('#record').append("<td>"+data.TVOC+"</td>")//TVOC
									$('#record').append("<td></td>")//CO2
									$('#record').append("<td>"+data.CO2+"</td>")//HCHO
									$('#record').append("<td>"+data.SO2+"</td>")//SO2
...
<div style="position:absolute;right:2%;top:0%;width:80%;height:100%;background-color:#e5eef7;border-radius:12px">
  ...
  <div style="position:absolute;left:24%;top:2%;width:20%;height:50px;">设备编号:
   <select id="DeviceID" style="width:70%;height:100%;border:1px solid #e6eef8;border-radius:6px;outline:none">
	<option value="1#" selected>1号酸性气体空气净化机</option>
	<option value="2#">2号酸性气体空气净化机</option>
	<option value="3#">3号酸性气体空气净化机</option>
	<option value="4#">4号酸性气体空气净化机</option>
	<option value="5#">5号酸性气体空气净化机</option>
   </select>
  </div>
