...
export default {
  ...
  methods: {
      ...
      axios
        ...
        .then(
          (success) => {
            ...
            //   }
            // console.log("循环结束");
            if (this.numberOfDevices[0]?.value == undefined) {
              return this.$message({
                message: "该控制器没有配置设备,请前往设备中心设置设备",
                type: "warning",
              });
            } else {
              this.DeviceID = this.numberOfDevices[0].value;
            }
    ...
    getdata(indata, ctlname) {
      ...
      // $("#btndsclose").css("color", "#ffffff");
      // $("#btndsclose").css("background-color", "#036ff6");
      // $("#btndsopen").css("color", "#036ff6");
      // $("#btndsopen").css("background-color", "#ffffff");
      ...
      axios
        ...
        .then(
          (success) => {
            ...
            $("#dataTXZT").html(
              obj1.data3[DeviceID].dataTXZT == 1 ? "离线" : "在线"
            );
            $("#datatemp").html(
              "<i style='color:black'>" +
                obj1.data3[DeviceID].datatemp +
                "<small>°C</small></i>"
            );
            $("#pm2_5").html(
              "<i style='color:black'>" +
                obj1.data3[DeviceID].datapm2_5 +
                "<small>ug/m³</small></i>"
            );
            $("#datahum").html(
              "<i style='color:black'>" +
                obj1.data3[DeviceID].datahum +
                "<small>%RH</small></i>"
            );
            $("#CO2").html(
              "<i style='color:black'>" +
                obj1.data3[DeviceID].dataCO2 +
                "<small>ppm</small></i>"
            );
            var fsstate = obj1.data3[DeviceID].dataFSZT;
            var fslist = new Array("关", "低", "中", "高");
            $("#fsstate").html(fslist[fsstate]);
            this.UpdateFSStates(fsstate);
            if (obj1.data3[DeviceID].dataGZMS == 1) {
            ...
            if (obj1.data3[DeviceID].datapowerZT == 1) {
            ...
            if (
              obj1.data3[DeviceID].dataenableTiming == "undefined" ||
              obj1.data3[DeviceID].dataenableTiming == 0
            ) {
              $("#btndsclose").css("color", "#ffffff");
              $("#btndsclose").css("background-color", "#036ff6");
              $("#btndsopen").css("color", "#036ff6");
              ...
              $("#btndsopen").css("color", "#ffffff");
              $("#btndsopen").css("background-color", "#036ff6");
              $("#btndsclose").css("color", "#036ff6");
              $("#btndsclose").css("background-color", "#ffffff");
            }
          },
          (error) => {
    ...
    btndsopen() {
      ...
      var indata = {
        ...
        hostname: "10.168.1.39",
        port: "8066",
        method: "UnmannedKFPTService/setBGSXFJTimerValue",
        jsonin:
          '{"ip":"' +
          $("#HostName").html() +
          '","deviceType":"3","deviceNo":"' +
          $("#datadeviceNo").html() +
          '","onOff":"1","openHour": "4","openMinute": "23","closeHour": "8","closeMinute": "40"}',
      };
      axios
        .post(
          "http://" + this.ServeIp + ":" + this.ServePort + "/getRfid",
    ...
    btndsclose() {
      ...
      var indata = {
        ...
        jsonin:
          ...
          '","deviceType":"3","deviceNo":"' +
          $("#datadeviceNo").html() +
          '","onOff":"0","openHour": "4","openMinute": "23","closeHour": "8","closeMinute": "40"}',
      };
      axios
        .post(
          "http://" + this.ServeIp + ":" + this.ServePort + "/getRfid",
