  ...
  <el-container style="height: 100%; border: 1px solid #eee">
    ...
    >
      ...
      <div
        ...
        "
      >
        窗帘状态数：<span id="dataCLZTS">0</span>
      </div>
      <div
        style="
          position: absolute;
      ...
      >
        ...
        <!-- <el-option label="净化除酸型除湿加湿一体机" value="1"></el-option>
								...
								<el-option label="智能电动窗帘" value="11" selected></el-option>
      ...
      >
        ...
        >
        <!-- <option value="0" selected>电动窗帘1</option>
								<option value="1">电动窗帘2</option>
								<option value="2">电动窗帘3</option>
								<option value="3">电动窗帘4</option>
								<option value="4">电动窗帘5</option>
								<option value="5">电动窗帘6</option>
								<option value="6">电动窗帘7</option>
								<option value="7">电动窗帘8</option>
								<option value="8">电动窗帘9</option>
								<option value="9">电动窗帘10</option>
								<option value="10">电动窗帘11</option>
								<option value="11">电动窗帘12</option> -->
      </el-select>
    </div>
  </el-container>
</template>
...
export default {
  ...
  methods: {
      ...
      axios
        ...
        .then(
          (success) => {
            ...
            let device = Object.keys(success.data.data.devices.ZNDDCL).length;
            for (let i = 1; i < device + 1; i++) {
              let deviceFacility = success.data.data.devices.ZNDDCL[i];
      ...
      axios
        ...
        .then(
          (success) => {
            ...
            console.log(JSON.stringify(obj1.data11[DeviceID]));
            $("#datadeviceNo").html(obj1.data11[DeviceID].datadeviceNo);
            $("#dataTXZT").html(
              obj1.data11[DeviceID].dataTXZT == 1 ? "离线" : "在线"
            );
            $("#dataCLZTS").html(
              "<i>" + obj1.data11[DeviceID].dataCLZTS + "<small>°%</small></i>"
            );
            if (obj1.data11[DeviceID].dataCLZTS == 0) {
              $("#btnopen").css({
                color: "#ffffff",
                background: "#036ff6",
              });
            ...
            } else if (obj1.data11[DeviceID].dataCLZTS == 50) {
              $("#btnopen").css({
                color: "#036ff6",
                background: "#ffffff",
              });
            ...
            } else if (obj1.data11[DeviceID].dataCLZTS == 100) {
              $("#btnopen").css({
                color: "#036ff6",
                background: "#ffffff",
              });
    ...
    btnstop() {
      ...
      $("#btnstop").css("color", "#ffffff");
      $("#btnstop").css("background-color", "#036ff6");
      $("#btnclose").css("color", "#036ff6");
      $("#btnclose").css("background-color", "#ffffff");
      var indata = {
        cmd: 60001,
