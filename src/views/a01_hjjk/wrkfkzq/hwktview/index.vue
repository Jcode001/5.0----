	...
	<el-container style="height: 100%; border: 1px solid #eee">
		<div style="position:absolute;right:2%;top:0%;width:98%;height:100%;background-color:#e5eef7;border-radius:12px">
			...
			<div
				...
				<el-button type="primary" size="big" @click="devicePage(`/hjjk/wrkfkzq/hwktview/index`)"
					disabled>设备页面</el-button>
				<el-button type="primary" size="big" @click="devicePage(`/hjjk/wrkfkzq/hwktview/report`)">数据报表</el-button>
				<el-button type="primary" size="big" @click="devicePage(`/hjjk/wrkfkzq/hwktview/chart`)">数据曲线</el-button>
			</div>
			...
			<div
				style="position:absolute;left:7%;top:49%;width:60%;height:40px;border:0px solid red;font-size:20px;line-height:40px">
				手动控制&emsp;
				<el-button id="btnclose" @click="btnclose()"
					style="width:10%;height:100%;border:3px solid #036ff6;color:#ffffff;background-color:#036ff6;border-radius:20px;font-size:18px;outline:medium">关机</el-button>
				<el-button id="btnopen" @click="btnopen()"
					style="width:10%;height:100%;border:3px solid #036ff6;color:#036ff6;background-color:#ffffff;border-radius:20px;font-size:18px;outline:medium">制冷</el-button>
				<el-button id="btnopen2" @click="btnopen2()"
					style="width:10%;height:100%;border:3px solid #036ff6;color:#036ff6;background-color:#ffffff;border-radius:20px;font-size:18px;outline:medium">制热</el-button>
				&emsp;&emsp;&emsp;&emsp;
				模&emsp;式&emsp;
			...
			<div
				style="position:absolute;left:7%;top:62.5%;width:60%;height:40px;border:0px solid red;font-size:20px;line-height:40px">
				运行状态：&emsp;
				<span style="color:#036ff6;" id="airStatus"></span>
			</div>
			<!-- <div
				style="position:absolute;left:7%;top:70%;width:60%;height:40px;border:0px solid red;font-size:20px;line-height:40px;">
				电流限值&emsp;
				<input id="dataDLMX" type="text"
					style="width:16%;height:100%;color:#9f9f9f;background-color:#ebf4fb;border:2px solid #9f9f9f;text-align:center;font-size:18px;"
					value="0" />
				<span style="color:#9f9f9f;">A</span>
				&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&ensp;
			</div> -->
			<div
				style="position:absolute;left:7%;top:75%;width:60%;height:40px;border:0px solid red;font-size:20px;line-height:40px;">
				温度范围&emsp;
			...
			<el-select v-model="DeviceType"
				...
				<!-- <el-option label="净化除酸型除湿加湿一体机" value="1"></el-option>
								...
								<el-option label="红外空调" value="8" selected></el-option>
			...
			<el-select v-model="DeviceID" @change="showAuto()" id="DeviceID"
				...
				<!-- <option value="0" selected>1号红外空调</option>
								<option value="1">2号红外空调</option>
								<option value="2">3号红外空调</option>
								<option value="3">4号红外空调</option>
								<option value="4">5号红外空调</option>
								<option value="5">6号红外空调</option>
								<option value="6">7号红外空调</option>
								<option value="7">8号红外空调</option>
								<option value="8">9号红外空调</option>
								<option value="9">10号红外空调</option>
								<option value="10">11号红外空调</option>
								<option value="11">12号红外空调</option>
								<option value="12">13号红外空调</option>
								<option value="13">14号红外空调</option>
								<option value="14">15号红外空调</option>
								<option value="15">16号红外空调</option> -->
			</el-select>
		</div>
	</el-container>
</template>
	...
	methods: {
		...
		getdata(indata, ctlname) {
			...
			axios.post('http://' + this.ServeIp + ":" + this.ServePort + '/getRfid', indata).then(
				success => {
					...
					console.log(JSON.stringify(obj1.data8[DeviceID]))
					$("#datadeviceNo").html(obj1.data8[DeviceID].datadeviceNo);
					$("#dataTXZT").html(obj1.data8[DeviceID].dataTXZT == 1 ? "离线" : "在线");
					$("#datatemp").html("<i style='color:black'>" + obj1.data8[DeviceID].datatemp + "<small>°C</small></i>");
					$("#datahum").html("<i style='color:black'>" + obj1.data8[DeviceID].datahum + "<small>%RH</small></i>");

					$("#minTemp").val(obj1.data8[DeviceID].dataminTemp);
					$("#maxTemp").val(obj1.data8[DeviceID].datamaxTemp);

					$("#dataDLMX").val(obj1.data8[DeviceID].dataDLMX);
					var airStatuslist = new Array("关", "制冷", "制热");
					$("#airStatus").html(airStatuslist[obj1.data8[DeviceID].dataairStatus]);
					if (obj1.data8[DeviceID].dataGZMS == 1) {
					...
					if (obj1.data8[DeviceID].dataairStatus == 0) {
						$("#btnclose").css("color", "#ffffff");
						$("#btnclose").css("background-color", "#036ff6");
						$("#btnopen").css("color", "#036ff6");
						$("#btnopen").css("background-color", "#ffffff");
						$("#btnopen2").css("color", "#036ff6");
						$("#btnopen2").css("background-color", "#ffffff");
					} else if (obj1.data8[DeviceID].dataairStatus == 1) {
						...
						$("#btnopen2").css("color", "#036ff6");
						$("#btnopen2").css("background-color", "#ffffff");
					} else if (obj1.data8[DeviceID].dataairStatus == 2) {
						$("#btnopen2").css("color", "#ffffff");
						$("#btnopen2").css("background-color", "#036ff6");
						$("#btnopen").css("color", "#036ff6");
						$("#btnopen").css("background-color", "#ffffff");
						$("#btnclose").css("color", "#036ff6");
						$("#btnclose").css("background-color", "#ffffff");
		...
		btnopen() {
			...
			var indata = {
				...
				"jsonin": "{\"ip\":\"" + $('#HostName').html() + "\",\"deviceType\":\"8\",\"deviceNo\":\"" + $("#datadeviceNo").html() + "\",\"operaFlag\":\"1\"}"
			}
			axios.post('http://' + this.ServeIp + ":" + this.ServePort + '/getRfid', indata).then(
				success => {
					let Obj = success.data;
		...
		btnopen2() {
			...
			var indata = {
				...
				"port": "8066",
				"method": "UnmannedKFPTService/setUnmannedPower",
				"jsonin": "{\"ip\":\"" + $('#HostName').html() + "\",\"deviceType\":\"8\",\"deviceNo\":\"" + $("#datadeviceNo").html() + "\",\"operaFlag\":\"2\"}"
			}
			axios.post('http://' + this.ServeIp + ":" + this.ServePort + '/getRfid', indata).then(
				success => {
					let Obj = success.data;
		...
		btnclose() {
			...
			var indata = {
				...
				"port": "8066",
				"method": "UnmannedKFPTService/setUnmannedPower",
				"jsonin": "{\"ip\":\"" + $('#HostName').html() + "\",\"deviceType\":\"8\",\"deviceNo\":\"" + $("#datadeviceNo").html() + "\",\"operaFlag\":0}"
			}
			axios.post('http://' + this.ServeIp + ":" + this.ServePort + '/getRfid', indata).then(
				success => {
					let Obj = success.data;
		...
		btnTempSet() {
			var indata = {
				...
				"jsonin": "{\"ip\":\"" + $('#HostName').html() + "\",\"deviceType\":8,\"deviceNo\":\"" + $("#datadeviceNo").html() + "\",\"minTemp\":" + $("#minTemp").val() + ",\"maxTemp\":" + $("#maxTemp").val() + "}"
			}
			axios.post('http://' + this.ServeIp + ":" + this.ServePort + '/getRfid', indata).then(
				success => {
					let Obj = success.data;
