	...
	<el-container style="height: 100%; border: 1px solid #eee">
		<div style="position:absolute;right:2%;top:0%;width:98%;height:100%;background-color:#e5eef7;border-radius:12px">
			...
			<div
				...
				<el-button type="primary" size="big" @click="devicePage(`/hjjk/wrkfkzq/jkytfhjview/index`)"
					disabled>设备页面</el-button>
				<el-button type="primary" size="big"
					@click="devicePage(`/hjjk/wrkfkzq/jkytfhjview/report`)">数据报表</el-button>
				<el-button type="primary" size="big" @click="devicePage(`/hjjk/wrkfkzq/jkytfhjview/chart`)">数据曲线</el-button>
			</div>
			<span style="position:absolute;left:12.3%;top:31%;font-size:31px;color:#036ff6" id="datatemp"></span>
			<span style="position:absolute;left:32%;top:31%;font-size:26px;color:#036ff6" id="pm2_5"></span>
			<span style="position:absolute;left:52%;top:31%;font-size:26px;color:#036ff6" id="datahum"></span>
			<span style="position:absolute;left:24%;top:53%;font-size:16px;color:#036ff6" id="CO2"></span>
			<div
				style="position:absolute;left:7%;top:65%;width:60%;height:40px;border:0px solid red;font-size:20px;line-height:40px">
				开关机&emsp;
				<el-button id="btnopen" @click="btnopen()"
					style="width:10%;height:100%;border:3px solid #036ff6;color:#ffffff;background-color:#036ff6;font-size:18px;border-radius:20px;">开</el-button>
				<el-button id="btnclose" @click="btnclose()"
					style="width:10%;height:100%;border:3px solid #036ff6;color:#036ff6;background-color:#ffffff;font-size:18px;border-radius:20px;">关</el-button>
				&emsp;&emsp;&emsp;&emsp;&emsp;
				模&emsp;式&emsp;
				<el-button id="btnmode1" @click="btnmode1()"
					style="width:10%;height:100%;border:3px solid #036ff6;color:#ffffff;background-color:#036ff6;font-size:18px;border-radius:20px;">手动</el-button>
				<el-button id="btnmode2" @click="btnmode2()"
					style="width:10%;height:100%;border:3px solid #036ff6;color:#036ff6;background-color:#ffffff;font-size:18px;border-radius:20px;">自动</el-button>
			</div>
			<div
				style="position:absolute;left:7%;top:76%;width:60%;height:40px;border:0px solid red;font-size:20px;line-height:40px">
				恒时灯&emsp;
				<el-button id="btnhsdopen" @click="btnhsdopen()"
					style="width:10%;height:100%;border:3px solid #036ff6;color:#ffffff;background-color:#036ff6;font-size:18px;border-radius:20px;">开</el-button>
				<el-button id="btnhsdclose" @click="btnhsdclose()"
					style="width:10%;height:100%;border:3px solid #036ff6;color:#036ff6;background-color:#ffffff;font-size:18px;border-radius:20px;">关</el-button>
				&emsp;&emsp;&emsp;&emsp;&emsp;
				风&emsp;速&emsp;
			...
			<el-select v-model="DeviceType"
				...
				<!-- <el-option label="净化除酸型除湿加湿一体机" value="1" selected></el-option>
								<el-option label="健康防护一体机" value="2" selected></el-option>
			...
			<el-select v-model="DeviceID" @change="showAuto()" id="DeviceID"
				...
				<!-- <option value="0" selected>1号健康防护一体机</option>
								<option value="1">2号健康防护一体机</option>
								<option value="2">3号健康防护一体机</option>
								<option value="3">4号健康防护一体机</option>
								<option value="4">5号健康防护一体机</option>
								<option value="5">6号健康防护一体机</option>
								<option value="6">7号健康防护一体机</option>
								<option value="7">8号健康防护一体机</option>
								<option value="8">9号健康防护一体机</option>
								<option value="9">10号健康防护一体机</option> -->
			</el-select>
		</div>
	</el-container>
</template>
...
export default {
	...
	methods: {
		...
		deviceInformation() {
			...
			axios.get("http://" + this.ServeIp + ":" + this.ServePort + `/settingDeviceShow/getAll?warehouseId=${sessionStorage.getItem("StoreId")}&controllerId=${sessionStorage.getItem("ACId")}`).then(
				success => {
					...
					let device = Object.keys(success.data.data.devices.JKFHYTJ).length;
					for (let i = 1; i < device + 1; i++) {
						let deviceFacility = success.data.data.devices.JKFHYTJ[i];
		...
		getdata(indata, ctlname) {
			...
			// $("#btnhsdclose").css("color", "#ffffff");
			// $("#btnhsdclose").css("background-color", "#036ff6");
			// $("#btnhsdopen").css("color", "#036ff6");
			// $("#btnhsdopen").css("background-color", "#ffffff");
			axios.post('http://' + this.ServeIp + ":" + this.ServePort + '/getRfid', indata).then(
				success => {
					...
					$("#datatemp").html("<i style='color:black'>" + obj1.data2[DeviceID].datatemp + "<small>°C</small></i>");
					$("#pm2_5").html("<i style='color:black'>" + obj1.data2[DeviceID].datapm2_5 + "<small>ug/m³</small></i>");
					$("#datahum").html("<i style='color:black'>" + obj1.data2[DeviceID].datahum + "<small>%RH</small></i>");
					$("#CO2").html("<i style='color:black'>" + obj1.data2[DeviceID].dataCO2 + "<small>ppm</small></i>");

					// var fsstate = obj1.data2[DeviceID].dataFSZT;
					// var fslist = new Array("关", "低", "中", "高");
					// $("#fsstate").html(fslist[fsstate]);
					// if (obj1.data2[DeviceID].dataFSZT == 0) {
					// 	$("#fsstate0").css("color", "#ffffff");
					// 	$("#fsstate0").css("background-color", "#036ff6");
					// 	$("#fsstate1").css("color", "#036ff6");
					// 	$("#fsstate1").css("background-color", "#ffffff");
					...
					// 	$("#fsstate3").css("color", "#036ff6");
					// 	$("#fsstate3").css("background-color", "#ffffff");
					// } else if (obj1.data2[DeviceID].dataFSZT == 1) {
					...
					// 	$("#fsstate3").css("color", "#036ff6");
					// 	$("#fsstate3").css("background-color", "#ffffff");
					// } else if (obj1.data2[DeviceID].dataFSZT == 3) {
					// 	$("#fsstate0").css("color", "#036ff6");
					// 	$("#fsstate0").css("background-color", "#ffffff");
					// 	$("#fsstate1").css("color", "#036ff6");
					// 	$("#fsstate1").css("background-color", "#ffffff");
					...
					// 	$("#fsstate3").css("color", "#036ff6");
					// 	$("#fsstate3").css("background-color", "#ffffff");
					// } else if (obj1.data2[DeviceID].dataFSZT == 5) {
					// 	$("#fsstate0").css("color", "#036ff6");
					// 	$("#fsstate0").css("background-color", "#ffffff");
					// 	$("#fsstate1").css("color", "#036ff6");
					// 	$("#fsstate1").css("background-color", "#ffffff");
					...
					// }
					if (obj1.data2[DeviceID].dataGZMS == 1) {
					...
					if (obj1.data2[DeviceID].datapowerZT == 1) {
					...
					if (obj1.data2[DeviceID].dataHSD == 1) {
						$("#btnhsdopen").css("color", "#ffffff");
						$("#btnhsdopen").css("background-color", "#036ff6");
						$("#btnhsdclose").css("color", "#036ff6");
						$("#btnhsdclose").css("background-color", "#ffffff");
					} else {
						$("#btnhsdclose").css("color", "#ffffff");
						$("#btnhsdclose").css("background-color", "#036ff6");
						$("#btnhsdopen").css("color", "#036ff6");
		...
		btnopen() {
			...
			var indata = {
				...
				"jsonin": "{\"ip\":\"" + $('#HostName').html() + "\",\"deviceType\":2,\"deviceNo\":\"" + $("#datadeviceNo").html() + "\",\"operaFlag\":1}"
			}
			axios.post('http://' + this.ServeIp + ":" + this.ServePort + '/getRfid', indata).then(
				success => {
					let Obj = success.data;
		...
		btnclose() {
			...
			var indata = {
				...
				"port": "8066",
				"method": "UnmannedKFPTService/setUnmannedPower",
				"jsonin": "{\"ip\":\"" + $('#HostName').html() + "\",\"deviceType\":2,\"deviceNo\":\"" + $("#datadeviceNo").html() + "\",\"operaFlag\":0}"
			}
			axios.post('http://' + this.ServeIp + ":" + this.ServePort + '/getRfid', indata).then(
				success => {
					let Obj = success.data;
		...
		btnhsdopen() {
			...
			var indata = {
				...
				"hostname": "10.168.1.39",
				"port": "8066",
				"method": "UnmannedKFPTService/setUnmannedHSD",
				"jsonin": "{\"ip\":\"" + $('#HostName').html() + "\",\"deviceType\":2,\"deviceNo\":\"" + $("#datadeviceNo").html() + "\",\"operaFlag\":1}"
			}
			axios.post('http://' + this.ServeIp + ":" + this.ServePort + '/getRfid', indata).then(
				success => {
		...
		fsstate0() {
			...
			var indata = {
				...
				"jsonin": "{\"ip\":\"" + $('#HostName').html() + "\",\"deviceType\":2,\"deviceNo\":\"" + $("#datadeviceNo").html() + "\",\"operaFlag\":" + operaFlag + "}"
			}
			axios.post('http://' + this.ServeIp + ":" + this.ServePort + '/getRfid', indata).then(
				success => {
					let Obj = success.data;
