	...
	<el-container style="height: 100%; border: 1px solid #eee">
		<div style="position:absolute;right:2%;top:0%;width:98%;height:100%;background-color:#e5eef7;border-radius:12px">
			...
			<div
				...
				<el-button type="primary" size="big" @click="devicePage(`/hjjk/wrkfkzq/jsqview/index`)"
					disabled>设备页面</el-button>
				<el-button type="primary" size="big" @click="devicePage(`/hjjk/wrkfkzq/jsqview/report`)">数据报表</el-button>
				<el-button type="primary" size="big" @click="devicePage(`/hjjk/wrkfkzq/jsqview/chart`)">数据曲线</el-button>
			</div>
			<span style="position:absolute;left:12.7%;top:31%;font-size:30px;color:#036ff6" id="datatemp"></span>
			...
			<div
				...
				<el-button id="btnopen" @click="btnopen()"
					style="width:10%;height:100%;border:3px solid #036ff6;color:#ffffff;background-color:#036ff6;border-radius:20px;font-size:20px;outline:medium">开</el-button>
				<el-button id="btnclose" @click="btnclose()"
					style="width:10%;height:100%;border:3px solid #036ff6;color:#036ff6;background-color:#ffffff;border-radius:20px;font-size:20px;outline:medium">关</el-button>
				&emsp;&emsp;&emsp;
				模&emsp;式
				<el-button id="btnmode1" @click="btnmode1()"
					style="width:10%;height:100%;border:3px solid #036ff6;color:#ffffff;background-color:#036ff6;border-radius:20px;font-size:20px;outline:medium">手动</el-button>
				<el-button id="btnmode2" @click="btnmode2()"
					style="width:10%;height:100%;border:3px solid #036ff6;color:#036ff6;background-color:#ffffff;border-radius:20px;font-size:20px;outline:medium">自动</el-button>
				&emsp;&emsp;&emsp;
				加湿状态：
			...
			<!-- <div
				...
				<el-button id="fsstate0" @click="fsstate0()"
					style="width:5%;height:100%;border:3px solid #036ff6;color:#ffffff;background-color:#036ff6;border-radius:20px 0 0 20px;font-size:20px;outline:medium">关</el-button>
				<el-button id="fsstate1" @click="fsstate1()"
					style="width:5%;height:100%;border:3px solid #036ff6;color:#036ff6;background-color:#ffffff;font-size:20px;outline:medium">低</el-button>
				<el-button id="fsstate2" @click="fsstate2()"
					style="width:5%;height:100%;border:3px solid #036ff6;color:#036ff6;background-color:#ffffff;font-size:20px;outline:medium">中</el-button>
				<el-button id="fsstate3" @click="fsstate3()"
					style="width:5%;height:100%;border:3px solid #036ff6;color:#036ff6;background-color:#ffffff;font-size:20px;border-radius:0 20px 20px 0;outline:medium">高</el-button>
			</div> -->
			<div
				style="position:absolute;left:7%;top:73%;width:60%;height:40px;border:0px solid red;font-size:20px;line-height:40px;">
				湿度下限
				...
				<button id="btnHumSet" @click="btnHumSet"
					style="width:10%;height:100%;border:0;color:#ffffff;background-color:#036ff6;font-size:18px;border-radius:20px;">确认</button>
			</div>
			<el-select v-model="DeviceType"
				...
				<!-- <el-option label="净化除酸型除湿加湿一体机" value="1"></el-option>
								...
								<el-option label="加湿机" value="5" selected></el-option>
			...
			<el-select v-model="DeviceID" @change="showAuto()" id="DeviceID"
				...
				<!-- <option value="0" selected>1号加湿机</option>
								<option value="1">2号加湿机</option>
								<option value="2">3号加湿机</option>
								<option value="3">4号加湿机</option>
								<option value="4">5号加湿机</option>
								<option value="5">6号加湿机</option>
								<option value="6">7号加湿机</option>
								<option value="7">8号加湿机</option>
								<option value="8">9号加湿机</option>
								<option value="9">10号加湿机</option>
								<option value="10">11号加湿机</option>
								<option value="11">12号加湿机</option> -->
			</el-select>
		</div>
	</el-container>
</template>
	...
	methods: {
		...
		deviceInformation() {
			...
			axios.get("http://" + this.ServeIp + ":" + this.ServePort + `/settingDeviceShow/getAll?warehouseId=${sessionStorage.getItem("StoreId")}&controllerId=${sessionStorage.getItem("ACId")}`).then(
				success => {
					...
					let device = Object.keys(success.data.data.devices.JSJ).length;
					for (let i = 1; i < device + 1; i++) {
						let deviceFacility = success.data.data.devices.JSJ[i];
		...
		getdata(indata, ctlname) {
			...
			axios.post('http://' + this.ServeIp + ":" + this.ServePort + '/getRfid', indata).then(
				success => {
					...
					console.log(JSON.stringify(obj1.data5[DeviceID]))
					$("#datadeviceNo").html(obj1.data5[DeviceID].datadeviceNo);
					$("#dataTXZT").html(obj1.data5[DeviceID].dataTXZT == 1 ? "离线" : "在线");
					$("#datatemp").html("<i  style='color:black'>" + obj1.data5[DeviceID].datatemp + "<small>°C</small></i>");
					$("#datahum").html("<i  style='color:black'>" + obj1.data5[DeviceID].datahum + "<small>%RH</small></i>");
					$("#pm2_5").html("<i  style='color:black'>" + obj1.data5[DeviceID].datapm2_5 + "<small>ug/m³</small></i>");
					$("#dataCSZT").html(obj1.data5[DeviceID].dataCS == 1 ? "开启" : "关闭");
					$("#minHum").val(obj1.data5[DeviceID].dataJSSD);

					if (obj1.data5[DeviceID].dataSXZT == 0) {
						var alarm = "水空";
					} else if (obj1.data5[DeviceID].dataSXZT == 1) {
						var alarm = "正常";
					} else if (obj1.data5[DeviceID].dataSXZT == 2) {
						var alarm = "空白";
					} else if (obj1.data5[DeviceID].dataSXZT == 3) {
						var alarm = "水满";
					}
					$("#dataCSZT").text(alarm)
					var fsstate = obj1.data5[DeviceID].dataFJZT;
					this.UpdateFSStates(fsstate);
					if (obj1.data5[DeviceID].datapowerZT == 1) {
					...
					if (obj1.data5[DeviceID].dataGZMS == 1) {
		...
		btnopen() {
			...
			var indata = {
				...
				"jsonin": "{\"ip\":\"" + $('#HostName').html() + "\",\"deviceType\":5,\"deviceNo\":\"" + $("#datadeviceNo").html() + "\",\"operaFlag\":1}"
			}
			axios.post('http://' + this.ServeIp + ":" + this.ServePort + '/getRfid', indata).then(
				success => {
					let Obj = success.data;
		...
		btnclose() {
			...
			var indata = {
				...
				"port": "8066",
				"method": "UnmannedKFPTService/setUnmannedPower",
				"jsonin": "{\"ip\":\"" + $('#HostName').html() + "\",\"deviceType\":\"5\",\"deviceNo\":\"" + $("#datadeviceNo").html() + "\",\"operaFlag\":\"0\"}"
			}
			axios.post('http://' + this.ServeIp + ":" + this.ServePort + '/getRfid', indata).then(
				success => {
					let Obj = success.data;
		...
		fsstate0() {
			...
			var indata = {
				...
				"jsonin": "{\"ip\":\"" + $('#HostName').html() + "\",\"deviceType\":5,\"deviceNo\":\"" + $("#datadeviceNo").html() + "\",\"operaFlag\":" + operaFlag + "}"
			}
			axios.post('http://' + this.ServeIp + ":" + this.ServePort + '/getRfid', indata).then(
				success => {
					let Obj = success.data;
		...
		btnHumSet() {
			var indata = {
				...
				"jsonin": "{\"ip\":\"" + $('#HostName').html() + "\",\"deviceType\": \"5\",\"deviceNo\": \"" + $("#datadeviceNo").html() + "\",\"minHum\":" + $("#minHum").val() + "}"
			}
			axios.post('http://' + this.ServeIp + ":" + this.ServePort + '/getRfid', indata).then(
				success => {
					let Obj = success.data;
