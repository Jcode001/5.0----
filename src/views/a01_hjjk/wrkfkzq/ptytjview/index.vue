	...
	<el-container style="height: 100%; border: 1px solid #eee">
		<div style="position:absolute;right:2%;top:0%;width:98%;height:100%;background-color:#e5eef7;border-radius:12px">
			...
			<div
				...
				<el-button type="primary" size="big" @click="devicePage(`/hjjk/wrkfkzq/ptytjview/index`)"
					disabled>设备页面</el-button>
				<el-button type="primary" size="big" @click="devicePage(`/hjjk/wrkfkzq/ptytjview/report`)">数据报表</el-button>
				<el-button type="primary" size="big" @click="devicePage(`/hjjk/wrkfkzq/ptytjview/chart`)">数据曲线</el-button>
			</div>
			<span style="position:absolute;left:14%;top:33%;font-size:30px;color:#036ff6" id="datatemp"></span>
			<span style="position:absolute;left:33%;top:33%;font-size:28px;color:#036ff6" id="pm2_5"></span>
			<span style="position:absolute;left:53%;top:33%;font-size:28px;color:#036ff6" id="datahum"></span>
			<div
			...
			<div
				...
				<!-- 主净化
				<el-button id="btnjhopen" @click="btnjhopen()"
					style="width:10%;height:100%;border:3px solid #036ff6;color:#ffffff;background-color:#036ff6;border-radius:20px;font-size:18px;outline:medium">开</el-button>
				<el-button id="btnjhclose" @click="btnjhclose()"
					style="width:10%;height:100%;border:3px solid #036ff6;color:#036ff6;background-color:#ffffff;border-radius:20px;font-size:18px;outline:medium">关</el-button>
				&emsp;&emsp;&emsp; -->
				风&emsp;速
				...
				<el-button id="fsstate3" @click="fsstate3()"
					style="width:5%;height:100%;border:3px solid #036ff6;color:#036ff6;background-color:#ffffff;border-radius:0 20px 20px 0;font-size:18px;outline:medium">高</el-button>
				&emsp;&emsp;&emsp;&emsp;
				加湿状态：
			...
			<el-select v-model="DeviceType"
				...
				<!-- <el-option label="净化除酸型除湿加湿一体机" value="1"></el-option>
								...
								<el-option label="普通一体机" value="7" selected></el-option>
			...
			<el-select v-model="DeviceID" @change="showAuto()" id="DeviceID"
				...
				<!-- <option value="0" selected>1号普通一体机</option>
								<option value="1">2号普通一体机</option>
								<option value="2">3号普通一体机</option>
								<option value="3">4号普通一体机</option>
								<option value="4">5号普通一体机</option>
								<option value="5">6号普通一体机</option>
								<option value="6">7号普通一体机</option>
								<option value="7">8号普通一体机</option>
								<option value="8">9号普通一体机</option>
								<option value="9">10号普通一体机</option>
								<option value="10">11号普通一体机</option>
								<option value="11">12号普通一体机</option> -->
	...
	methods: {
		...
		deviceInformation() {
			...
			axios.get("http://" + this.ServeIp + ":" + this.ServePort + `/settingDeviceShow/getAll?warehouseId=${sessionStorage.getItem("StoreId")}&controllerId=${sessionStorage.getItem("ACId")}`).then(
				success => {
					...
					let device = Object.keys(success.data.data.devices.PTYTJ).length;
					for (let i = 1; i < device + 1; i++) {
						let deviceFacility = success.data.data.devices.PTYTJ[i];
		...
		getdata(indata, ctlname) {
			...
			axios.post('http://' + this.ServeIp + ":" + this.ServePort + '/getRfid', indata).then(
				success => {
					...
					var fsstate = obj1.data7[DeviceID].dataFJZT;
					...
					$("#datatemp").html("<i style='color:black'>" + obj1.data7[DeviceID].datatemp + "<small>°C</small></i>");
					$("#pm2_5").html("<i style='color:black'>" + obj1.data7[DeviceID].datapm2_5 + "<small>ug/m³</small></i>");
					$("#datahum").html("<i style='color:black'>" + obj1.data7[DeviceID].datahum + "<small>%RH</small></i>");

					if (obj1.data7[DeviceID].datapowerZT == 1) {
					...
					if (obj1.data7[DeviceID].dataGZMS == 1) {
					...
					if (obj1.data7[DeviceID].dataZJH == 1) {
					...
					$("#dataCSZT").html(obj1.data7[DeviceID].dataCSZT == 1 ? "开启" : "关闭");
					$("#dataJSZT").html(obj1.data7[DeviceID].dataJSZT == 1 ? "开启" : "关闭");
					$("#dataminHum").val(obj1.data7[DeviceID].dataJSSD);
					$("#datamaxHum").val(obj1.data7[DeviceID].dataCSSD);
					var SXZTlist = new Array("空", "正常", "未知", "满");
					$("#dataSXZT").html(SXZTlist[obj1.data1[DeviceID].dataSXZT]);

					var alarm = "正常";
					if (obj1.data7[DeviceID].dataZBJ == 1)
						alarm = "报警";

					$("#dataLSBJ").html(alarm);

					var runstates;
					if (obj1.data7[DeviceID].dataPSTX == 1)
						runstates = "排水";
					else if (obj1.data7[DeviceID].dataJSTX == 1)
						runstates = "加水";
					else
						runstates = "正常";
		...
		btnopen() {
			...
			var indata = {
				...
				"jsonin": "{\"ip\":\"" + $('#HostName').html() + "\",\"deviceType\":\"7\",\"deviceNo\":\"" + $("#datadeviceNo").html() + "\",\"operaFlag\":\"1\"}"
			}
			axios.post('http://' + this.ServeIp + ":" + this.ServePort + '/getRfid', indata).then(
				success => {
					let Obj = success.data;
		...
		btnclose() {
			...
			var indata = {
				...
				"port": "8066",
				"method": "UnmannedKFPTService/setUnmannedPower",
				"jsonin": "{\"ip\":\"" + $('#HostName').html() + "\",\"deviceType\":\"7\",\"deviceNo\":\"" + $("#datadeviceNo").html() + "\",\"operaFlag\":\"0\"}"
			}
			axios.post('http://' + this.ServeIp + ":" + this.ServePort + '/getRfid', indata).then(
				success => {
					let Obj = success.data;
		...
		btnjhopen() {
			...
			var indata = {
				...
				"hostname": "10.168.1.39",
				"port": "8066",
				"method": "UnmannedKFPTService/setUnmannedJH",
				"jsonin": "{\"ip\":\"" + $('#HostName').html() + "\",\"deviceType\":7,\"deviceNo\":\"" + $("#datadeviceNo").html() + "\",\"operaFlag\":1}"
			}
			axios.post('http://' + this.ServeIp + ":" + this.ServePort + '/getRfid', indata).then(
				success => {
		...
		fsstate0() {
			...
			var indata = {
				...
				"jsonin": "{\"ip\":\"" + $('#HostName').html() + "\",\"deviceType\":7,\"deviceNo\":\"" + $("#datadeviceNo").html() + "\",\"operaFlag\":" + operaFlag + "}"
			}
			axios.post('http://' + this.ServeIp + ":" + this.ServePort + '/getRfid', indata).then(
				success => {
					let Obj = success.data;
		...
		btnHumSet() {
			var indata = {
				...
				"jsonin": "{\"ip\":\"" + $('#HostName').html() + "\",\"deviceType\":7,\"deviceNo\":\"" + $("#datadeviceNo").html() + "\",\"minHum\":" + $("#dataminHum").val() + ",\"maxHum\":" + $("#datamaxHum").val() + "}"
			}
			axios.post('http://' + this.ServeIp + ":" + this.ServePort + '/getRfid', indata).then(
				success => {
					let Obj = success.data;
