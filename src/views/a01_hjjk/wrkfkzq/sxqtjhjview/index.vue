    ...
    >
      ...
      >
        <el-button
          type="primary"
          size="big"
          @click="devicePage(`/hjjk/wrkfkzq/sxqtjhjview/index`)"
          disabled
          >设备页面</el-button
        >
        <el-button
          type="primary"
          size="big"
          @click="devicePage(`/hjjk/wrkfkzq/sxqtjhjview/report`)"
          >数据报表</el-button
        >
        <el-button
          type="primary"
          size="big"
          @click="devicePage(`/hjjk/wrkfkzq/sxqtjhjview/chart`)"
          >数据曲线</el-button
        >
      </div>
      <span
      ...
      >
        ...
        <!-- <el-option label="净化除酸型除湿加湿一体机" value="1"></el-option>
								...
								<el-option label="酸性气体空气净化机" value="4" selected></el-option>
      ...
      <div
        ...
        "
      >
        设备编号：<span id="datadeviceNo"></span> &emsp;&emsp;通讯状态：<span
          id="dataTXZT"
        ></span>
      ...
      >
        <el-option
          v-for="items in numberOfDevices"
          :value="items.value"
          :key="items.value"
          :label="items.name"
        ...
        <!-- <option value="0" selected>1号酸性气体空气净化机</option>
								...
								<option value="5">6号酸性气体空气净化机</option>
								<option value="6">7号酸性气体空气净化机</option>
								<option value="7">8号酸性气体空气净化机</option>
								<option value="8">9号酸性气体空气净化机</option>
								<option value="9">10号酸性气体空气净化机</option>
								<option value="10">11号酸性气体空气净化机</option>
								<option value="11">12号酸性气体空气净化机</option> -->
      </el-select>
    </div>
  </el-container>
</template>
...
export default {
  ...
  mounted() {
    if (
      ...
      sessionStorage.getItem("ACName") != undefined &&
      sessionStorage.getItem("HostName") != undefined
    ) {
  ...
  methods: {
      ...
      axios
        .get(
          "http://" +
            ...
            ":" +
            this.ServePort +
            `/settingDeviceShow/getAll?warehouseId=${sessionStorage.getItem(
              "StoreId"
            )}&controllerId=${sessionStorage.getItem("ACId")}`
        )
        .then(
          (success) => {
            ...
            let device = Object.keys(success.data.data.devices.CSXKQJHJ).length;
            for (let i = 1; i < device + 1; i++) {
              let deviceFacility = success.data.data.devices.CSXKQJHJ[i];
            ...
            for (
              let i = 0;
              i < Object.keys(success.data.data.devices).length;
              i++
            ) {
      ...
      axios
        ...
        .then(
          (success) => {
            ...
            $("#dataTXZT").html(
              obj1.data4[DeviceID].dataTXZT == 1 ? "离线" : "在线"
            );
            $("#datatemp").html(
              "<i style='color:black'>" +
                obj1.data4[DeviceID].datatemp +
                "<small>°C</small></i>"
            );
            $("#pm2_5").html(
              "<i style='color:black'>" +
                obj1.data4[DeviceID].datapm2_5 +
                "<small>ug/m³</small></i>"
            );
            $("#datahum").html(
              "<i style='color:black'>" +
                obj1.data4[DeviceID].datahum +
                "<small>%RH</small></i>"
            );
            $("#TVOC").html(
              "<i style='color:black'>" +
                obj1.data4[DeviceID].dataTVOC +
                "<small>mg/m³</small></i>"
            );
            $("#SO2").html(
              "<i style='color:black'>" +
                obj1.data4[DeviceID].dataSO2 +
                "<small>ug/m³</small></i>"
            );
            $("#HCHO").html(
              "<i style='color:black'>" +
                obj1.data4[DeviceID].dataHCHO +
                "<small>mg/m³</small></i>"
            );

            var fsstate = obj1.data4[DeviceID].dataFSZT;
            this.UpdateFSStates(fsstate);
            if (obj1.data4[DeviceID].datapowerZT == 1) {
            ...
            if (obj1.data4[DeviceID].dataGZMS == 1) {
            ...
            if (obj1.data4[DeviceID].dataZR == 1) {
            ...
            if (obj1.data4[DeviceID].dataZJHZT == 1) {
    ...
    btnzropen() {
      ...
      var indata = {
        ...
        hostname: "10.168.1.39",
        port: "8066",
        method: "UnmannedKFPTService/setUnmannedCSXKQJHJZR",
        jsonin:
          '{"ip":"' +
          $("#HostName").html() +
          '","deviceType":4,"deviceNo":"' +
