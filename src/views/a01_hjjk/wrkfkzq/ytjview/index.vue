	...
	<el-container style="height: 100%; border: 1px solid #eee">
		<div style="position:absolute;right:2%;top:0%;width:98%;height:100%;background-color:#e5eef7;border-radius:12px">
			...
			<div
				...
				<el-button type="primary" size="big" @click="devicePage(`/hjjk/wrkfkzq/ytjview/index`)"
					disabled>设备页面</el-button>
				<el-button type="primary" size="big" @click="devicePage(`/hjjk/wrkfkzq/ytjview/report`)">数据报表</el-button>
				<el-button type="primary" size="big" @click="devicePage(`/hjjk/wrkfkzq/ytjview/chart`)">数据曲线</el-button>
			</div>
			<span style="position:absolute;left:13.5%;top:33%;font-size:31px;color:#036ff6" id="datatemp"></span>
			...
			<el-select v-model="DeviceID"
				...
				<!-- <el-option value="0" label="1号净化除酸型除湿加湿一体机"  selected></el-option>
				<el-option value="1" label="2号净化除酸型除湿加湿一体机"></el-option>
				<el-option value="2" label="3号净化除酸型除湿加湿一体机"></el-option>
				<el-option value="3" label="4号净化除酸型除湿加湿一体机"></el-option>
				<el-option value="4" label="5号净化除酸型除湿加湿一体机"></el-option>
				<el-option value="5" label="6号净化除酸型除湿加湿一体机"></el-option>
				<el-option value="6" label="7号净化除酸型除湿加湿一体机"></el-option>
				<el-option value="7" label="8号净化除酸型除湿加湿一体机"></el-option>
				<el-option value="8" label="9号净化除酸型除湿加湿一体机"></el-option>
				<el-option value="9" label="10号净化除酸型除湿加湿一体机"></el-option> -->
			</el-select>
		</div>
	</el-container>
</template>
		...
		getdata(indata, ctlname) {
			...
			axios.post('http://' + this.ServeIp + ":" + this.ServePort + '/getRfid', indata).then(
				success => {
					...
					let fsstate = obj1.data1[DeviceID].dataFJZT;
					...
					$("#datatemp").html("<i style='color:black'>" + obj1.data1[DeviceID].datatemp + "<small>°C</small></i>");
					$("#pm2_5").html("<i style='color:black'>" + obj1.data1[DeviceID].datapm2_5 + "<small>ug/m³</small></i>");
					$("#datahum").html("<i style='color:black'>" + obj1.data1[DeviceID].datahum + "<small>%RH</small></i>");
					...
					$("#dataCSZT").html(obj1.data1[DeviceID].dataCS == 1 ? "开启" : "关闭");
					$("#dataJSZT").html(obj1.data1[DeviceID].dataJS == 1 ? "开启" : "关闭");

					var SXZTlist = new Array("空", "正常", "未知", "满");
					$("#dataSXZT").html(SXZTlist[obj1.data1[DeviceID].dataSXZT]);

					var alarm = "正常";
					if (obj1.data1[DeviceID].dataZBJ == 1)
						alarm = "报警";
