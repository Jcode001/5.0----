		...
		<div style="display:flex;">
			...
			<div
				...
				<div
					...
					<el-button type="primary" size="big" @click="devicePage(`/hjjk/wrkfkzq/ytjview/index`)">设备页面</el-button>
					<el-button type="primary" size="big" @click="devicePage(`/hjjk/wrkfkzq/ytjview/report`)"
						disabled>数据报表</el-button>
					<el-button type="primary" size="big" @click="devicePage(`/hjjk/wrkfkzq/ytjview/chart`)">数据曲线</el-button>
				...
				<div style="position:absolute;left:24%;top:2%;width:20%;height:50px;">设备编号:
					<el-select v-model="DeviceID"
						...
						<!-- <el-option value="1#" label="1号净化除酸型除湿加湿一体机"  selected></el-option>
					<el-option value="2#" label="2号净化除酸型除湿加湿一体机"  ></el-option>
					<el-option value="3#" label="3号净化除酸型除湿加湿一体机"  ></el-option>
					<el-option value="4#" label="4号净化除酸型除湿加湿一体机"  ></el-option>
					<el-option value="5#" label="5号净化除酸型除湿加湿一体机"  ></el-option>
					<el-option value="6#" label="6号净化除酸型除湿加湿一体机"  ></el-option>
					<el-option value="7#" label="7号净化除酸型除湿加湿一体机"  ></el-option>
					<el-option value="8#" label="8号净化除酸型除湿加湿一体机"  ></el-option>
					<el-option value="9#" label="9号净化除酸型除湿加湿一体机"  ></el-option>
					<el-option value="10#" label="10号净化除酸型除湿加湿一体机"  ></el-option>
					<el-option value="11#" label="11号净化除酸型除湿加湿一体机"  ></el-option>
					<el-option value="12#" label="12号净化除酸型除湿加湿一体机"  ></el-option>
					<el-option value="13#" label="13号净化除酸型除湿加湿一体机"  ></el-option>
					<el-option value="14#" label="14号净化除酸型除湿加湿一体机"  ></el-option>
					<el-option value="15#" label="15号净化除酸型除湿加湿一体机"  ></el-option>
					<el-option value="16#" label="16号净化除酸型除湿加湿一体机"  ></el-option> -->
					</el-select>
				</div>
				<div style="position:absolute;left:2.5%;top:10%;width:95%;height:80%;overflow-y:auto;">
					...
					<el-table :data="tableData" style="position:relative;width:98%;" border id="ZJDAdaochu">
						...
						<el-table-column prop="hum" label="湿度(%RH)" align="center">
						</el-table-column>
						<el-table-column prop="PM1" label="PM1(ug/m³)" align="center">
						</el-table-column>
						<el-table-column prop="PM2_5" label="PM2.5(ug/m³)" align="center">
						</el-table-column>
						<el-table-column prop="PM10" label="PM10(ug/m³)" align="center">
						</el-table-column>
						<el-table-column prop="TVOC" label="TVOC(mg/m³)" align="center">
						</el-table-column>
						<el-table-column prop="CO2" label="CO₂(ppm)" align="center">
						</el-table-column>
						<el-table-column prop="HCHO" label="HCHO(mg/m³)" align="center">
						</el-table-column>
						<el-table-column prop="SO2" label="SO₂(mg/m³)" align="center">
						</el-table-column>
						<el-table-column prop="insert_time" label="记录时间" :formatter="dateFormat" align="center">
						</el-table-column>
					</el-table>
				</div>
...
export default {
	...
	methods: {
		...
		DA_zaijiedaochu() {
			...
			try {
				FileSaver.saveAs(
					...
					`净化除酸型除湿加湿一体机${year}${mounth}${day}${hours}${minutes}${seconds}` + ".xlsx"
				);
			} catch (e) {
		...
		deviceInformation() {
			...
			axios.get("http://" + this.ServeIp + ":" + this.ServePort + `/settingDeviceShow/getAll?warehouseId=${sessionStorage.getItem("StoreId")}&controllerId=${sessionStorage.getItem("ACId")}`).then(
				success => {
					...
					let device = Object.keys(success.data.data.devices.CSXJSCSYTJ).length;
					for (let i = 1; i < device + 1; i++) {
						let deviceFacility = success.data.data.devices.CSXJSCSYTJ[i];
		...
		showAuto() {
			...
			indata['jsonin'] = '{"ip":"' + $('#HostName').html() + '","deviceType":"1","deviceNo":"' + this.DeviceID + '","startDate": "' + startdate + '","endDate": "' + enddate + '"}';
			//console.log(indata)
