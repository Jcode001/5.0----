    ...
    <el-container style="height: calc(100vh - 120px);">
      <el-aside :class="newblue ? 'newclassoneone' : 'oldclassoneone'">
        ...
        <el-header style="text-align: left; border-radius:12px 12px 0 0;">
          ...
          <span
            style="vertical-align:middle;width:90%;color:#fff;text-align:left;font-size:1vw;line-height: 100%;padding-left: 10px;">档案分类</span>
        </el-header>
    ...
    width: 9.9vw;height: 100%;">
            <el-scrollbar style="" wrap-class="scrollbar-wrapper">
              <el-tree style="font-size:14px;height:69.5vh;overflow-y: scroll;color:#000000" :data="FLList"
                :props="defaultProps" highlight-current @node-click="kfdahandleNodeClick">
        ...
        <el-dialog title="模型设计" center :visible.sync="ModelDesignVisible">
          <el-tabs v-model="ModelDesignActiveName" @tab-click="handleClick">
            <el-tab-pane label="档案盒" name="1">
              ...
              <el-table  border height="400" :data="DAH_HookList" @selection-change="DAH_handleSelectionChange">
            ...
            <el-tab-pane label="档案" name="2">
              ...
              <el-table border height="400" :data="DAHookList" @selection-change="handleSelectionChange">
                <el-table-column type="selection" width="55"></el-table-column>
                <el-table-column property="index" label="序号" width="150"></el-table-column>
                <el-table-column property="name" label="挂接项名称" width="200"></el-table-column>
                <el-table-column property="type" label="挂接项类型" width="150"></el-table-column>
        ...
        <el-dialog :title=DATableDesign center :visible.sync="SubTableDesignVisible">
          <el-tabs v-model="activeNameone" @tab-click="handleClickone">
            <el-tab-pane label="档案盒" name="first">
              <div class="Modeldesign">
                <div style="margin-left: -1vw"><el-button type="primary" @click="getDAH_RecordList()">刷新</el-button></div>
                <div>
                  <el-button type="primary" @click="zlx_dah_add_Visible = true; getgjxoptions_hgl(0);">添加</el-button>
                  <el-dialog width="30%" title="添加档案盒著录项" :visible.sync="zlx_dah_add_Visible" append-to-body>
                    <el-form>
                      <el-form :model="dfrmDAZlxAdd_hgl">
                        <el-form-item label="序号：" :label-width="formLabelWidth">
                          <el-input v-model="dfrmDAZlxAdd_hgl.xsort" autocomplete="off" clearable=""></el-input>
                        </el-form-item>
                        <el-form-item label="字段英文名称：" :label-width="formLabelWidth">
                          <el-input v-model="dfrmDAZlxAdd_hgl.xname" autocomplete="off" clearable=""></el-input>
                        </el-form-item>
                        <el-form-item label="字段中文名称：" :label-width="formLabelWidth">
                          <el-input v-model="dfrmDAZlxAdd_hgl.xbyname" autocomplete="off" clearable=""></el-input>
                        </el-form-item>
                        <el-form-item label="字段类型：" :label-width="formLabelWidth">
                          <el-select v-model="dfrmDAZlxAdd_hgl.xlx" clearable placeholder="请选择" style="width: 100%;"
                            @change="getgjxoptions_hgl(dfrmDAZlxAdd_hgl.xlx)">
                            <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value">
                            </el-option>
                          </el-select>
                        </el-form-item>
                        <el-form-item label="约束性：" :label-width="formLabelWidth">
                          <el-select v-model="dfrmDAZlxAdd_hgl.isnotnull" clearable placeholder="请选择"
                            style="width: 100%;">
                            <el-option v-for="item in ysxoptions" :label="item.label" :value="item.value"></el-option>
                          </el-select>
                        </el-form-item>
                        <el-form-item label="字段长度：" :label-width="formLabelWidth">
                          <el-input v-model="dfrmDAZlxAdd_hgl.maxlength" autocomplete="off" clearable=""></el-input>
                        </el-form-item>
                        <el-form-item label="列表宽度：" :label-width="formLabelWidth">
                          <el-input v-model="dfrmDAZlxAdd_hgl.colwidth" autocomplete="off" clearable=""></el-input>
                        </el-form-item>
                        <el-form-item label="挂接项：" :label-width="formLabelWidth">
                          <el-select v-model="dfrmDAZlxAdd_hgl.gjxname" clearable placeholder="请选择" style="width: 100%;">
                            <el-option v-for="item in gjxoptions_hgl" :key="item.label" :label="item.label"
                              :value="item.label">
                            </el-option>
                          </el-select>
                        </el-form-item>
                        <el-form-item label="RFID挂接项：" :label-width="formLabelWidth">
                          <!-- <el-select v-model="dfrmDAZlxAdd_hgl.rfidgjx" placeholder="请选择">
                          <el-option   label="无挂接" value="0"></el-option>
                          <el-option   label="信息项" value="1"></el-option>
                          <el-option   label="编号项" value="2"></el-option>
                        </el-select> -->
                          <el-select v-model="dfrmDAZlxAdd_hgl.rfidgjx" clearable placeholder="请选择" style="width: 100%;">
                            <el-option v-for="item in rfidgjxoptions" :key="item.label" :label="item.label"
                              :value="item.label">
                            </el-option>
                          </el-select>
                        </el-form-item>
                        <el-form-item label="对齐方式：" :label-width="formLabelWidth">
                          <el-select v-model="dfrmDAZlxAdd_hgl.halign" placeholder="请选择" style="width: 100%;">
                            <el-option label="左对齐" value="0" selected></el-option>
                    ...
                    <div slot="footer" class="dialog-footer" style="text-aline: center">
                      <el-button @click="zlx_dah_add_Visible = false">取 消</el-button>
                      <el-button type="primary" @click="addSumbitDAH_ZLX()">添 加</el-button>
                    </div>
                  </el-dialog>
                </div>

                <div><el-button type="danger" @click="editDAH_Record()">修改</el-button>
                  <el-dialog   width="30%" title="修改档案盒著录项" :visible.sync="zlx_dah_edit_Visible" append-to-body>
                    <el-form>
                      <el-form :model="dfrmDAZlxedit_hgl">
                        <el-form-item label="序号：" :label-width="formLabelWidth">
                          <el-input v-model="dfrmDAZlxedit_hgl.xsort" autocomplete="off" clearable=""></el-input>
                        </el-form-item>
                        <el-form-item label="字段英文名称：" :label-width="formLabelWidth">
                          <el-tag type="danger" style="width: 100%;">{{ dfrmDAZlxedit_hgl.xname }}</el-tag>
                        </el-form-item>
                        <el-form-item label="字段中文名称：" :label-width="formLabelWidth">
                          <el-input v-model="dfrmDAZlxedit_hgl.xbyname" autocomplete="off" clearable=""></el-input>
                        </el-form-item>
                        <el-form-item label="字段类型：" :label-width="formLabelWidth">
                          <el-select v-model="dfrmDAZlxedit_hgl.xlx" clearable placeholder="请选择" style="width: 100%;"
                            @change="getgjxoptions_hgl(dfrmDAZlxedit_hgl.xlx)">
                            <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value">
                            </el-option>
                          </el-select>
                        </el-form-item>
                        <el-form-item label="约束性：" :label-width="formLabelWidth">
                          <el-select v-model="dfrmDAZlxedit_hgl.isnotnull" clearable placeholder="请选择"
                            style="width: 100%;">
                            <el-option v-for="item in ysxoptions" :label="item.label" :value="item.value"></el-option>
                          </el-select>
                        </el-form-item>
                        <el-form-item label="字段长度：" :label-width="formLabelWidth">
                          <el-input v-model="dfrmDAZlxedit_hgl.maxlength" autocomplete="off" clearable=""></el-input>
                        </el-form-item>
                        <el-form-item label="列表宽度：" :label-width="formLabelWidth">
                          <el-input v-model="dfrmDAZlxedit_hgl.colwidth" autocomplete="off" clearable=""></el-input>
                        </el-form-item>
                        <el-form-item label="挂接项：" :label-width="formLabelWidth">
                          <el-select v-model="dfrmDAZlxedit_hgl.gjxname" clearable placeholder="请选择" style="width: 100%;">
                            <el-option v-for="item in gjxoptions_hgl" :key="item.label" :label="item.label"
                              :value="item.label">
                            </el-option>
                          </el-select>
                        ...
                        <el-form-item label="RFID挂接项：" :label-width="formLabelWidth">
                          <el-select v-model="dfrmDAZlxedit_hgl.rfidgjx" clearable placeholder="请选择" style="width: 100%;">
                            <el-option v-for="item in rfidgjxoptions" :key="item.label" :label="item.label"
                              :value="item.label">
                            </el-option>
                          </el-select>
                        ...
                        <el-form-item label="对齐方式：" :label-width="formLabelWidth">
                          <el-select v-model="dfrmDAZlxedit_hgl.halign" placeholder="请选择" style="width: 100%;">
                            <el-option label="左对齐" value="0" selected></el-option>
                            <el-option label="右对齐" value="1"></el-option>
                            <el-option label="居中" value="2"></el-option>
                          </el-select>
                    ...
                    <div slot="footer" class="dialog-footer" style="text-aline: center">
                      <el-button @click="zlx_dah_edit_Visible = false">取 消</el-button>
                      <el-button type="primary" @click="editSumbitDAH_ZLX()">更 新</el-button>
                    </div>
                  </el-dialog>
                </div>
                <div><el-button type="info" @click="deleteDAH_Record()">删除</el-button></div>
                <div style="margin-left: 1vw">
                  <el-button type="primary" @click="DAHSyncTbStructure()">同步表结构</el-button>
                </div>
                <!-- <div style="margin-left: 1vw">
                  <el-button type="primary">更换表结构</el-button>
                </div> -->
                <div style="margin-left: 1vw">
                  <el-button type="primary" @click="clearalldata()">清空当前档案门类下档案盒数据</el-button>
                </div>
              </div>
              <el-table height="400" :data="DAH_RecordList" ref="DAH_RecordTable"
                @selection-change="DAH_Record_handleSelectionChange" border="">
                <el-table-column type="selection" width="55"></el-table-column>
                <el-table-column property="index" label="序号" width="50"></el-table-column>
                <el-table-column property="record1" label="缩写" width="100"></el-table-column>
                ...
                <el-table-column property="colwidth" label="列宽"></el-table-column>
                <el-table-column property="need" label="约束性"></el-table-column>
            ...
            <el-tab-pane label="档案" name="second">
              <div class="Modeldesign">
                <div style="margin-left: -1vw"><el-button type="primary" @click="getDA_RecordList()">刷新</el-button></div>
                <div>
                  <el-button type="primary" @click="zlx_add_Visible = true; getgjxoptions(0);">添加</el-button>
                  <el-dialog width="30%" title="添加档案著录项" :visible.sync="zlx_add_Visible" append-to-body>
                    <el-form>
                      <el-form :model="dfrmDAZlxAdd">
                        <el-form-item label="序号：" :label-width="formLabelWidth">
                          <el-input v-model="dfrmDAZlxAdd.xsort" autocomplete="off" clearable=""></el-input>
                        </el-form-item>
                        <el-form-item label="字段英文名称：" :label-width="formLabelWidth">
                          <el-input v-model="dfrmDAZlxAdd.xname" autocomplete="off" clearable=""></el-input>
                        </el-form-item>
                        <el-form-item label="字段中文名称：" :label-width="formLabelWidth">
                          <el-input v-model="dfrmDAZlxAdd.xbyname" autocomplete="off" clearable=""></el-input>
                        </el-form-item>
                        <el-form-item label="字段类型：" :label-width="formLabelWidth">
                          <el-select v-model="dfrmDAZlxAdd.xlx" clearable placeholder="请选择" style="width: 100%;"
                            @change="getgjxoptions(dfrmDAZlxedit.xlx)">
                            <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value">
                            </el-option>
                          </el-select>
                        </el-form-item>
                        <el-form-item label="约束性：" :label-width="formLabelWidth">
                          <el-select v-model="dfrmDAZlxAdd.isnotnull" clearable placeholder="请选择" style="width: 100%;">
                            <el-option v-for="item in ysxoptions" :label="item.label" :value="item.value"></el-option>
                          </el-select>
                        </el-form-item>
                        <el-form-item label="字段长度：" :label-width="formLabelWidth">
                          <el-input v-model="dfrmDAZlxAdd.maxlength" autocomplete="off" clearable=""></el-input>
                        </el-form-item>
                        <el-form-item label="列表宽度：" :label-width="formLabelWidth">
                          <el-input v-model="dfrmDAZlxAdd.colwidth" autocomplete="off" clearable=""></el-input>
                        </el-form-item>
                        <el-form-item label="挂接项：" :label-width="formLabelWidth">
                          <el-select v-model="dfrmDAZlxAdd.gjxname" clearable placeholder="请选择" style="width: 100%;">
                            <el-option v-for="item in gjxoptions" :key="item.label" :label="item.label"
                              :value="item.label">
                            </el-option>
                          </el-select>
                        </el-form-item>
                        <el-form-item label="RFID挂接项：" :label-width="formLabelWidth">
                          ...
                          <el-select v-model="dfrmDAZlxAdd.rfidgjx" clearable placeholder="请选择" style="width: 100%;">
                            <el-option v-for="item in rfidgjxoptions" :key="item.label" :label="item.label"
                              :value="item.label">
                            </el-option>
                          </el-select>
                        ...
                        <el-form-item label="对齐方式：" :label-width="formLabelWidth">
                          <el-select v-model="dfrmDAZlxAdd.halign" placeholder="请选择" style="width: 100%;">
                            <el-option label="左对齐" value="0" selected></el-option>
                            <el-option label="右对齐" value="1"></el-option>
                            <el-option label="居中" value="2"></el-option>
                          </el-select>
                    ...
                    <div slot="footer" class="dialog-footer" style="text-aline: center">
                      <el-button @click="zlx_add_Visible = false">取 消</el-button>
                      <el-button type="primary" @click="addSumbitDA_ZLX()">添 加</el-button>
                    </div>
                  </el-dialog>
                </div>
                <div><el-button type="danger" @click="editDA_Record()">修改</el-button>
                  <el-dialog width="30%" title="修改档案著录项" :visible.sync="zlx_edit_Visible" append-to-body>
                    <el-form>
                      <el-form :model="dfrmDAZlxedit">
                        <el-form-item label="序号：" :label-width="formLabelWidth">
                          <el-input v-model="dfrmDAZlxedit.xsort" autocomplete="off" clearable=""></el-input>
                        </el-form-item>
                        <el-form-item label="字段英文名称：" :label-width="formLabelWidth">
                          <el-tag type="danger" style="width: 100%;">{{ dfrmDAZlxedit.xname }}</el-tag>
                        </el-form-item>
                        <el-form-item label="字段中文名称：" :label-width="formLabelWidth">
                          <el-input v-model="dfrmDAZlxedit.xbyname" autocomplete="off" clearable=""></el-input>
                        </el-form-item>
                        <el-form-item label="字段类型：" :label-width="formLabelWidth">
                          <el-select v-model="dfrmDAZlxedit.xlx" clearable placeholder="请选择" style="width: 100%;"
                            @change="getgjxoptions(dfrmDAZlxedit.xlx)">
                            <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value">
                            </el-option>
                          </el-select>
                        ...
                        <el-form-item label="约束性：" :label-width="formLabelWidth">
                          <el-select v-model="dfrmDAZlxedit.isnotnull" clearable placeholder="请选择" style="width: 100%;">
                            <el-option v-for="item in ysxoptions" :label="item.label" :value="item.value"></el-option>
                          </el-select>
                        </el-form-item>
                        <el-form-item label="字段长度：" :label-width="formLabelWidth">
                          <el-input v-model="dfrmDAZlxedit.maxlength" autocomplete="off" clearable=""></el-input>
                        </el-form-item>
                        <el-form-item label="列表宽度：" :label-width="formLabelWidth">
                          <el-input v-model="dfrmDAZlxedit.colwidth" autocomplete="off" clearable=""></el-input>
                        </el-form-item>
                        <el-form-item label="挂接项：" :label-width="formLabelWidth">
                          <el-select v-model="dfrmDAZlxedit.gjxname" clearable placeholder="请选择" style="width: 100%;">
                            <el-option v-for="item in gjxoptions" :key="item.label" :label="item.label"
                              :value="item.label">
                            </el-option>
                          </el-select>
                        ...
                        <el-form-item label="RFID挂接项：" :label-width="formLabelWidth">
                          <el-select v-model="dfrmDAZlxedit.rfidgjx" clearable placeholder="请选择" style="width: 100%;">
                            <el-option v-for="item in rfidgjxoptions" :key="item.label" :label="item.label"
                              :value="item.label">
                            </el-option>
                          </el-select>
                        ...
                        <el-form-item label="对齐方式：" :label-width="formLabelWidth">
                          <el-select v-model="dfrmDAZlxedit.halign" placeholder="请选择" style="width: 100%;">
                            <el-option label="左对齐" value="0" selected></el-option>
                            <el-option label="右对齐" value="1"></el-option>
                            <el-option label="居中" value="2"></el-option>
                          </el-select>
                    ...
                    <div slot="footer" class="dialog-footer" style="text-aline: center">
                      <el-button @click="zlx_edit_Visible = false">取 消</el-button>
                      <el-button type="primary" @click="editSumbitDA_ZLX()">更 新</el-button>
                    </div>
                  </el-dialog>
                </div>
                <div><el-button type="info" @click="deleteDA_Record()">删除</el-button></div>
                <div style="margin-left: 1vw">
                  <el-button type="primary" @click="DASyncTbStructure()">同步表结构</el-button>
                </div>
                <!-- <div style="margin-left: 1vw">
                  <el-button type="primary">更换表结构</el-button>
                </div> -->
                <div style="margin-left: 1vw">
                  <el-button type="primary" @click="clearalldata()">清空当前档案门类下档案数据</el-button>
                </div>
              </div>
              <el-table height="400" :data="DA_RecordList" @selection-change="DA_Record_handleSelectionChange" border="">
                <el-table-column type="selection" width="55"></el-table-column>
                <el-table-column property="index" label="序号" width="50"></el-table-column>
                <el-table-column property="record1" label="缩写" width="100"></el-table-column>
                <el-table-column property="record" label="著录项" width="100"></el-table-column>
     ...
     <div style="display:flex">
      ...
      <div style="display:flex;align-items: center;">
        档案盒项:
      <el-select v-model="valueoneoneDAH" placeholder="请选择自动装盒档案盒项">
          <el-option
...
return
    ...
    kfdahandleNodeClick(data) {//
      ...
      console.log(data.mlname, "档案门类ID:" + data.mlid, "父级：" + data.fatherkey, '点击了');
     bus.$off('DAoptions');
      ...
      const tempData = {};
      tempData.Table_name = "a01_hgldata";
      tempData.MLId = data.mlid;

      axios.post('http://' + this.ServeIp + ':' + this.ServePort + '/GetDAFLMLTableName', tempData).then(//根据MLID获取档案（盒）表名称
        success => {
          console.log(success, '根据MLID获取档案（盒）表名称-成功111111111111');
          if (success.data.code == 200)
            this.DATableDesignTittle = success.data.data
        ...
        axios.post('http://' + this.ServeIp + ':' + this.ServePort + '/GetZLXData1_hgl', dahlist).then(
        success => {
          console.log(success, '获取档案盒著录项列表数据-成功11');
          this.valueoneoneDAH = '';
          this.optionsDAH = [],

            success.data.data.forEach((item, index) => {
              this.optionsDAH.push({
                value: item.xbyname,
            ...
            axios.post('http://' + this.ServeIp + ':' + this.ServePort + '/GetZDZHConfig', dataone).then(//获取自动装盒名称
        success => {
          console.log(success, '根据表名称获取初始数据');
          // if (success.data.code == 200)
          if(success.data.code != 200){
            return
          }
          this.valueoneoorderby=success.data.data.orderby
          this.valueoneoneDA=success.data.data.xbyname
          this.valueoneoneDAH=success.data.data.xbyname_hgl

      
        },
        error => {
          console.log(error, '根据表名称获取初始数据--返回失败的数据');
             })
      ...
      bus.$emit("changeEvent", this.paramList.MLId, this.paramList.MLName, this.paramList.FatherKey, this.paramList.MLGS, this.danganjieyueflag,);
    },
    showMenu() {
    ...
    editSumbitDA_ZLX() {
      ...
      axios.post('http://' + this.ServeIp + ':' + this.ServePort + '/SaveZlxData', data).then(//添加或更新挂接项（档案盒）
        success => {
          console.log(success, '添加或更新著录项（档案）-成功');
          if (success.data.code == 200) {
            this.$message.success("更新成功")//添加或更新挂接项（档案盒）
