  ...
  <div  v-title data-title="智慧档案馆管理总平台 " class="app-container">
    ...
    <el-dialog title="在借档案" width="75%" center :visible.sync="filepopupwindow" @close="OnBrrowClose()">
      <el-tabs style="width:73vw" v-model="activeName" @tab-click="handleClick">
        <el-tab-pane label="档案盒" name="first">
          <div class="Modeldesign">
            ...
            <div><el-button type="primary" @click="weiyuqi">未逾期档案</el-button></div>
            <div><el-button type="primary" @click="heneiwenjian">盒内文件</el-button></div>
          </div>

          <el-table  ref="zaijielishitable"   border height="450" id="ZJDAHdaochu" @select-all="handleSelecthonghgl" :data="FileonloanTable"
            @selection-change="handleSelectionChangehonghgl">
            <el-table-column type="selection"></el-table-column>
            <el-table-column  show-overflow-tooltip :formatter="zaijieDAH" :prop="item.prop" :label="item.label" width="100"
              v-for="(item, index) in zaijielishihgl" :key="index"></el-table-column>
          </el-table>
        </el-tab-pane>
        ...
        <el-tab-pane label="档案" name="second">
          ...
          <el-table ref="zaijielishitable_DA"   border height="450" id="ZJDAdaochu" @select-all="handleSelecthongdangan" :data="borrowingfilebox"
      ...
      <div v-if="DAFYshow"  style="float: right;">
        <el-pagination @size-change="DAHFYhandleSizeChangeone" @current-change="DAHFYhandleCurrentChangeone"
          :current-page="DAHFYpIndex_da" :page-sizes="[2, 10, 100]" :page-size="DAHFYpSize_da"
      ...
      <div v-else  style="float: right;">
        <el-pagination @size-change="D_AFYhandleSizeChangeone" @current-change="D_AFYhandleCurrentChangeone"
          :current-page="D_AFYpIndex_da" :page-sizes="[2, 10, 100]" :page-size="D_AFYpSize_da"
    ...
    <el-dialog title="借阅历史记录" width="75%" center :visible.sync="borrowinghistory">
      ...
      <div v-if="jieyueDAFYshow"  style="float: right;">
        <el-pagination @size-change="jieyueDAHFYhandleSizeChangeone" @current-change="jieyueDAHFYhandleCurrentChangeone"
          :current-page="jieyueDAHFYpIndex_da" :page-sizes="[2, 10, 100]" :page-size="jieyueDAHFYpSize_da"
      ...
      <div v-else  style="float: right;">
        <el-pagination @size-change="jieyueD_AFYhandleSizeChangeone" @current-change="jieyueD_AFYhandleCurrentChangeone"
          :current-page="jieyueD_AFYpIndex_da" :page-sizes="[2, 10, 100]" :page-size="jieyueD_AFYpSize_da"
    ...
    <el-dialog @close="kuaisusaomaclose()" title="快速扫码" width="70%" center :visible.sync="quickcodescan">
      ...
      <div class="quickyle" style="margin-top: 1vh;">
        <el-table   ref="saomalist"  @row-click = "statussaoma" height="350" @selection-change="jieyuedanyuan" :data="quickygridData" border="">
          <el-table-column type="selection"> </el-table-column>
    ...
    <el-dialog title="通道门过门记录" width="70%" center :visible.sync="filepopupwindowRFID">
      ...
      <el-table border height="400" @select-all="handleSelect" @selection-change="handleSelectionChange_TDM" ref="multipleTable"
        :data="fileTableRFID" tooltip-effect="dark" style="width: 100%"> <el-table-column type="selection" width="55">
        </el-table-column>
      ...
      <div v-if="TDMdaiqueren"  style="float: right;">
        <el-pagination @size-change="TDMSizeChangeone" @current-change="TDMCurrentChangeone"
          :current-page="TDMDAHFYpIndex_da" :page-sizes="[2, 10, 100]" :page-size="TDMDAHFYpSize_da"
      ...
      <div v-else  style="float: right;">
        <el-pagination @size-change="WQRhandleSizeChangeone" @current-change="WQRYhandleCurrentChangeone"
          :current-page="WQRpIndex_da" :page-sizes="[2, 10, 100]" :page-size="WQRpSize_da"
    ...
    <el-dialog @close="pandianjihuaclose()" title="档案盘点计划" width="70%" center :visible.sync="filepopupwindowcheckcar">
      <div class="Modeldesign">
        ...
        <div>
          <el-input style="width: 15vw; margin-left: 1vw; margin-right: 0.5vw" v-model="filetelist.filepandian"
      ...
      <el-table :data="fileTableRFIDcheckcar" max-height="400" height="400" border
        @selection-change="handleSelectionPandian">
        ...
        <el-table-column  :show-overflow-tooltip='true' v-for="(item, idx) in DAcolumnInfoPDClist" :key="idx"
          :prop="item.prop" :label="item.label" width="180" header-align="center" align="center">
        </el-table-column>
      ...
      <div  v-if="pandianjisuashow"  style="float: right;">
        <el-pagination @size-change="pandiansizechange" @current-change="pandianCurrentChangeone"
          :current-page="pandianpIndex_da" :page-sizes="[2, 10, 100]" :page-size="pandianSize_da"
      ...
      <div  v-else style="float: right;">
        <el-pagination @size-change="searchsizechange" @current-change="searchCurrentChangeone"
          :current-page="searchpIndex_da" :page-sizes="[2, 10, 100]" :page-size="searchSize_da"
    ...
    <div class="header">
      <el-menu :default-active="activeIndex2" class="el-menu-demo" mode="horizontal" @select="handleSelect"
        ...
        <el-submenu index="2">
          <template slot="title"><i class="el-icon-s-fold"></i> <span style="color:#000">列表</span> </template>
        ...
        <el-submenu index="3">
          <template slot="title"><i class="el-icon-s-fold"></i><span style="color:#000"> 借阅管理</span></template>
        ...
        <el-menu-item index="7">
          <div @click="saomakuaisu" class="headerzhulu">
            <i class="el-icon-s-fold"></i>快速扫码
          </div>
        </el-menu-item>
        <el-submenu index="8">
          <template slot="title"><i class="el-icon-s-fold"></i> <span style="color:#000"> RFID</span></template>
    ...
    <div v-show="dapingshowone" class="containerone">
      ...
      <div class="TableHeader">
        <div>
          <div style="    margin-left: 1vw; margin-top: 1.3vw;"><img src="~@/assets/a02_dagl/档案盒.png" alt=""
              style="height: 1.9vh;"></div>
          <el-tag type="success" style="background:none;margin-right: 1vw;margin-left: -0.3vw;"></el-tag>
        </div>

        <div @mouseleave="shubiaoyichu()" @mouseenter="shubiaoyiru()"
          style="border-radius: 0.3vw;font-size:0.65vw;  height: 4.5vh;width: 4.5vw;display: flex;align-items: center;justify-content: center;color:#fff;background:#1890ff;margin-left:0.5vw"
          class="menuselect">
          菜单
        </div>
        ...
        <el-input
          ...
          v-model="inputlist" clearable=""></el-input>
      ...
      <div style="display:flex;position: relative;">
        ...
        <div @mouseenter="shubiaoyiru()" @mouseleave="shubiaoyichu()" v-show="yidonghezi"
          style="border: 1px solid #6792f0;z-index:999;width:40vw;height:12vh;background:#e0e4ec;position: absolute;top:-0.5vh;left:0vw">
          <div style="width:4vw;height:4vh;">
            <div style="display:flex;margin:1vw">
              ...
              <el-dialog title="标签打印" :visible.sync="PrinterCodeDialog" width="50%" height="500" center>
                <printer :IpPortPrinter_s="this.IpPortPrinter_s" v-if="PrinterCodeDialog" /> 
              </el-dialog>
              <!-- YJJ -->
            </div>
        ...
        <el-table id="table-listone" ref="multipleTable"
        @row-click = "btnDHA"
      ...
      <div style="margin-top: 0.3vw;float: right">
        <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="pIndex_dah"
          :page-sizes="[10, 50, 100]" :page-size=pSize_dah layout="total, sizes, prev, pager, next, jumper"
          :total="count_DAH">
        </el-pagination>
      </div>
    ...
    <div v-show="dapingshow" class="containertwo">
      <div class="TableHeader">
        <div>
          <div style="    margin-left: 1vw; margin-top: 1.3vw;"><img src="~@/assets/a02_dagl/档案卷.png" alt="" style="height: 1.9vh;"></div>
          <el-tag type="success" style="background:none;margin-right: 1vw;margin-left: -0.3vw;"></el-tag>
        </div>
        ...
        <el-input style="width: 10vw; margin-left: 1vw; margin-right: 1vw" v-model="DAinputlist" clearable=""></el-input>
      ...
      <div style="display:flex;position: relative;">
        ...
        <el-table  
        @row-click = "btnDA"
           id="table-listoneDA" :class="newclassone ? 'newclassone' : 'oldclassone'" height="400"
          ...
          <el-table-column width="150" fixed="right" label="操作" header-align="center" align="center">
            <template slot-scope="scope">
      ...
      <div style="float: right;">
        <el-pagination @size-change="handleSizeChangeone" @current-change="handleCurrentChangeone"
          :current-page="pIndex_da" :page-sizes="[10, 50, 100]" :page-size="pSize_da"
...
<!-- 档案高级搜索 -->
            ...
            <template v-for="(item, index) in RecordInputListearch">
              <el-descriptions-item :key="index">
                ...
                <el-select v-model="item.sel" style="width: 10vw;margin-left: auto;" placeholder="请选择著录项"
                  ...
                  <el-option v-for="(item, index) in DAoptions" :label="item.label" :key="index" :value="item.label"></el-option>
                </el-select>
                <el-select v-model="item.chse" style="width: 10vw;margin-left: auto;" placeholder="请选择判断条件">
                  <el-option label="等于" value="="></el-option>
                  <el-option label="不等于" value="<>"></el-option>
    ...
    <el-dialog  center title="盒内数据" :visible.sync="heneishuju" width="80%">
      <div style="height: 55vh;">
        ...
        <el-table   id="table-listoneDA"  height="500"
          @selection-change="handleSelectionChange" :data="kuaisujieyueheineitable" border ref="multipleTableone"
          style="width: 100%;font-size: 14px;overflow-x:visible; white-space: nowrap;">
          <el-table-column type="selection" header-align="center" align="center">
          </el-table-column>
...
console.log('获取标签的状态resresresresresresres失败');
    ...
    PrinterCode(){
        ...
        this.multiSelectallboxes.forEach(item => {
          tempParam.push({
            text:item.ctlj_ajjdh,
            code:item.CODE_HGL
          })
        });
        this.IpPortPrinter_s =localStorage.getItem('IpPrinter')+"/"+
          localStorage.getItem('PortPrinter')+"/"+this.multiSelectallboxes[0].ctlj_ajjdh+"/"+this.multiSelectallboxes[0].CODE_HGL+"/"
          +localStorage.getItem('IpPrinterServe')+"/"+localStorage.getItem('PortPrinterServe')+"/"+JSON.stringify(tempParam);//ip/端口/档号/RFID标签值
  ...
  'localip': "127.0.0.1"
   ...
   //档案盒左侧年度搜索
    collapse() {
      ...
      axios.post("http://" + this.ServeIp + ":" + this.ServePort + "/GetDAFenZu", arrset).then(
        (res) => {
          if(res.data.data==undefined){
            this.$message.error('暂无数据')
               return
          }
          console.log(res, '成功列表数据');
...
.el-dialog {
  ...
  // background-color: rgba(208, 211, 212)!important;
  // border: 1px solid rgb(13, 210, 236) !important;
}
