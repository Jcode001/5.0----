    ...
    >
      ...
      >
        <el-button
          type="primary"
          size="big"
          @click="devicePage(`/sfgz/wrkfkzq/gz_lsview/index`)"
          disabled
          >设备页面</el-button
        >
        <el-button
          type="primary"
          size="big"
          @click="devicePage(`/sfgz/wrkfkzq/gz_lsview/report`)"
          >数据报表</el-button
        >
      </div>
      <el-select
      ...
      >
        ...
        <!-- <el-option label="环境超限专用报警器" value="4" selected></el-option>
							...
							<el-option label="漏水报警器" value="1" selected></el-option>
      ...
      >
        ...
        <!-- <option value="0" selected>1号漏水报警器</option>
								...
								<option value="4">5号漏水报警器</option>
								<option value="5">6号漏水报警器</option>
								<option value="6">7号漏水报警器</option>
								<option value="7">8号漏水报警器</option> -->
      </el-select>
    </div>
  </el-container>
</template>
      ...
      axios
        ...
        .then(
          (success) => {
            ...
            console.log(JSON.stringify(obj1.data14[DeviceID]));
            $("#datadeviceNo").html(obj1.data14[DeviceID].datadeviceNo);
            $("#dataTXZT").html(
              obj1.data14[DeviceID].dataTXZT == 1 ? "离线" : "在线"
            );
            if (obj1.data14[DeviceID].dataisAlarm == "0")
              $("#dataalarm").attr("src", require("./pic0.png"));
            else $("#dataalarm").attr("src", require("./pic.png"));
          },
          (error) => {
		...
		function getdata(indata,ctlname)
		{
			$.ajax({
					...
					success: function(Obj,status){
						...
						console.log(JSON.stringify(obj1.data14[DeviceID]))
						$("#datadeviceNo").html(obj1.data14[DeviceID].datadeviceNo);
						$("#dataTXZT").html(obj1.data14[DeviceID].dataTXZT==1?"离线":"在线");
						if(obj1.data14[DeviceID].dataisAlarm=="0")
