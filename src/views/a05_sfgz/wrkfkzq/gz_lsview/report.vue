			...
			<div
				...
				<div
					...
					<el-button type="primary" size="big"
						@click="devicePage(`/sfgz/wrkfkzq/gz_lsview/index`)">设备页面</el-button>
					<el-button type="primary" size="big" @click="devicePage(`/sfgz/wrkfkzq/gz_lsview/report`)"
				...
				<div style="position:absolute;left:4%;top:2%;width:20%;height:50px;">设备类型:
					<el-select v-model="DeviceType"
						...
						<!-- <el-option label="环境超限专用报警器" value="4" ></el-option>
							...
							<el-option label="漏水报警器" value="14" selected></el-option>
				...
				<div style="position:absolute;left:24%;top:2%;width:20%;height:50px;">设备编号:
					<el-select id="DeviceID" v-model="DeviceID"
						...
						<!-- <el-option label="1号漏水报警器" value="0" selected></el-option>
							<el-option label="2号漏水报警器" value="1" ></el-option>
							<el-option label="3号漏水报警器" value="2" ></el-option>
							<el-option label="4号漏水报警器" value="3"></el-option>
							<el-option label="5号漏水报警器" value="4"></el-option>
							<el-option label="6号漏水报警器" value="5"></el-option>
							<el-option label="7号漏水报警器" value="6"></el-option>
							<el-option label="8号漏水报警器" value="7"></el-option>
                            <el-option label="9号漏水报警器" value="8" ></el-option>
							<el-option label="10号漏水报警器" value="9" ></el-option>
							<el-option label="11号漏水报警器" value="10"></el-option>
							<el-option label="12号漏水报警器" value="11"></el-option>
							<el-option label="13号漏水报警器" value="12"></el-option>
							<el-option label="14号漏水报警器" value="13"></el-option>
							<el-option label="15号漏水报警器" value="14"></el-option>
                            <el-option label="16号漏水报警器" value="15"></el-option> -->
					</el-select>
				</div>
...
export default {
	...
	methods: {
		...
		DA_zaijiedaochu() {
			...
			try {
				FileSaver.saveAs(
					...
					`漏水报警器${year}${mounth}${day}${hours}${minutes}${seconds}` + ".xlsx"
				);
			} catch (e) {
		...
		deviceInformation() {
			...
			axios.get("http://" + this.ServeIp + ":" + this.ServePort + `/settingDeviceShow/getAll?warehouseId=${sessionStorage.getItem("StoreId")}&controllerId=${sessionStorage.getItem("ACId")}`).then(
				success => {
					...
					let device = Object.keys(success.data.data.devices.LSMK).length;
					for (let i = 1; i < device + 1; i++) {
						let deviceFacility = success.data.data.devices.LSMK[i];
		...
		showAuto() {
			...
			indata['jsonin'] = '{"ip": "' + sessionStorage.getItem('HostName') + '","deviceType": "14","deviceNo": "' + this.DeviceID + '#","startDate": "' + startdate + '","endDate": "' + enddate + '"}';
			//console.log(indata)
					...
					if (Obj.flag == 1) {
						...
						for (let i = 0; i < Obj.data.list.length; i++) {
							...
							if (Obj.data.list[i].isAlarm == 1) {
								this.awacs = "漏水预警"
							} else if (Obj.data.list[i].isAlarm == 2) {
								this.awacs = "低压预警"
							} else if (Obj.data.list[i].isAlarm == 0) {
								this.awacs = "无"
							}
