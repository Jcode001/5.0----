	...
	<el-container style="height: 100%; border: 1px solid #eee">
		<div style="position:absolute;right:2%;top:0%;width:98%;height:100%;background-color:#e5eef7;border-radius:12px">
			...
			<div
				...
				<el-button type="primary" size="big" @click="devicePage(`/sfgz/wrkfkzq/gz_qsview/index`)"
					disabled>设备页面</el-button>
				<el-button type="primary" size="big" @click="devicePage(`/sfgz/wrkfkzq/gz_qsview/report`)">数据报表</el-button>
			</div>
			<el-select v-model="DeviceType" style="position:absolute;left:2.3%;top:3.8%;width:15%;height:40px;outline:none"
				...
				<!-- <el-option label="环境超限专用报警器" value="4" ></el-option>
							...
							<el-option label="驱鼠报警器" value="7" selected></el-option> -->
			</el-select>
			...
			<el-select id="DeviceID" v-model="DeviceID" @change="showAuto()"
				...
				<!-- <option value="0" selected>1号驱鼠报警器</option>
								<option value="1">2号驱鼠报警器</option>
								<option value="2">3号驱鼠报警器</option>
								<option value="3">4号驱鼠报警器</option>
								<option value="4">5号驱鼠报警器</option>
								<option value="5">6号驱鼠报警器</option>
								<option value="6">7号驱鼠报警器</option>
								<option value="7">8号驱鼠报警器</option> -->
			</el-select>
		</div>
	</el-container>
</template>
		...
		getdata(indata, ctlname) {
			...
			axios.post('http://' + this.ServeIp + ":" + this.ServePort + '/getRfid', indata).then(
				success => {
					...
					console.log(JSON.stringify(obj1.data19[DeviceID]))
					$("#datadeviceNo").html(obj1.data19[DeviceID].datadeviceNo);
					$("#dataTXZT").html(obj1.data19[DeviceID].dataTXZT == 1 ? "离线" : "在线");
					if (obj1.data19[DeviceID].dataFHBJ == "0")
						$("#dataalarm").attr('src', require('./pic0.png'));
					else
