            ...
            <div
                ...
                <div
                    ...
                    <el-button type="primary" size="big"
                        @click="devicePage(`/sfgz/wrkfkzq/gz_qsview/index`)">设备页面</el-button>
                    <el-button type="primary" size="big" @click="devicePage(`/sfgz/wrkfkzq/gz_qsview/report`)"
                ...
                <div style="position:absolute;left:24%;top:2%;width:20%;height:50px;">设备编号:
                    <el-select id="DeviceID" v-model="DeviceID"
                        ...
                        <!-- <el-option label="1号防盗报警器" value="0" selected></el-option>
							<el-option label="2号防盗报警器" value="1" ></el-option>
							<el-option label="3号防盗报警器" value="2" ></el-option>
							<el-option label="4号防盗报警器" value="3"></el-option>
							<el-option label="5号防盗报警器" value="4"></el-option>
							<el-option label="6号防盗报警器" value="5"></el-option>
							<el-option label="7号防盗报警器" value="6"></el-option>
							<el-option label="8号防盗报警器" value="7"></el-option>
                            <el-option label="9号防盗报警器" value="8" ></el-option>
							<el-option label="10号防盗报警器" value="9" ></el-option>
							<el-option label="11号防盗报警器" value="10"></el-option>
							<el-option label="12号防盗报警器" value="11"></el-option>
							<el-option label="13号防盗报警器" value="12"></el-option>
							<el-option label="14号防盗报警器" value="13"></el-option>
							<el-option label="15号防盗报警器" value="14"></el-option>
                            <el-option label="16号防盗报警器" value="15"></el-option> -->
                    </el-select>
                </div>
        ...
        DA_zaijiedaochu() {
            ...
            try {
                FileSaver.saveAs(
                    ...
                    `驱鼠报警器${year}${mounth}${day}${hours}${minutes}${seconds}` + ".xlsx"
                );
            } catch (e) {
        ...
        deviceInformation() {
            ...
            axios.get("http://" + this.ServeIp + ":" + this.ServePort + `/settingDeviceShow/getAll?warehouseId=${sessionStorage.getItem("StoreId")}&controllerId=${sessionStorage.getItem("ACId")}`).then(
                success => {
                    ...
                    let device = Object.keys(success.data.data.devices.FHQS).length;
                    for (let i = 1; i < device + 1; i++) {
                        let deviceFacility = success.data.data.devices.FHQS[i];
    ...
    <script>
        ...
        $(function () {
            ...
            $("#btnexport").bind("click", function () {
                ...
                var url = "/unmanned/fhai/exportToExcel13";
                ...
                params['requestParams'] = '{"ip": "' + HostName + '","deviceType": "19","deviceNo": "' + classifyList[index_One].arr[num].deviceSerialNumber + '#","startDate": "' + startdate + '","endDate": "' + enddate + '"}';
        ...
        $(document).ready(function () { //删除数据 start
            $("#empty-qr").click(function () {
                ...
                indata['jsonin'] = '{"ip":"' + HostName + '","type":"19"}';
                //console.log(indata)
                console.log(JSON.stringify(indata))
                $.ajax({
                    type: "POST",
...
<body style="margin:0;padding:0">
    ...
    <div style="position:absolute;right:2%;top:13%;width:80%;height:82%;background-color:#e5eef7;border-radius:12px">
        ...
        <div
            ...
            <a href="/twelvePrevent/toDevice?pageName=wrkfkzq/gz_qsview"><button
    ...
    <span style="width:90%;color:#bdc3d3;position:absolute;left:2%;top:8.5%;text-align:left;font-size:16px"><a
            target="_self" href="/viewMainMenu.html">档案馆管理总控平台</a>>档案库房环控</span>
