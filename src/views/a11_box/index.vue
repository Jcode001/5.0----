    ...
    <div v-title data-title="智慧档案馆管理总平台 " class="app-container">
        <el-dialog title="档案信息添加" @close="RecordClose()" width="50%" center :visible.sync="archivalinformation">
            <el-tabs v-model="activeNameRecord" @tab-click="Putdowntabs">
                <el-tab-pane label="档案箱" name="first">
                        ...
                        <el-descriptions title="基本信息" border>
                            <template v-for="(item, index) in RecordInputList">
                                ...
                                <el-descriptions-item v-if="item.type == 1" :key="index">
                                    ...
                                    <el-input type="number" style="width: 100%;margin-left: auto;"
        ...
        <el-dialog title="Excel普通导入" @close="importClose()" width="90%" center
            :visible.sync="archivalinformationImport">
            <div class="Modeldesign">
                        ...
                        <el-form :model="uploadlist_add">
                            <el-form-item label="Excel类型" :label-width="formLabelWidth">
                                <el-select v-model="uploadlist_add.exceltype" placeholder="请选择">
                                    <el-option label="档案箱" value="0"></el-option>
            ...
            <el-table :data="uploadlistdata" @selection-change="UpLoadList_handleSelectionChange" border
          ...
          <el-table-column fixed prop="noinportcnt" label="未导入条数" width="100" align="center"></el-table-column> -->
                ...
                <el-table-column fixed="right" label="操作" align="center">
                    ...
                    <template slot-scope="scope">
                        ...
                        <el-button type="text" size="small" style="margin-left:1rem"
                            @click="inportcheck(scope.row)">检查</el-button>
                        <el-button type="text" size="small" style="margin-left:1rem"
                            @click="inportdata(scope.row)">导入</el-button>
            ...
            <el-tabs style="width:73vw" v-model="activeName" @tab-click="handleClick">
                <el-tab-pane label="档案箱" name="first">
                    <div class="Modeldesign">
                        ...
                        <div><el-button type="primary" @click="yuqidanganhe">逾期档案箱</el-button></div>
                        <div><el-button type="primary" @click="weiyuqi">未逾期档案箱</el-button></div>
                        <div><el-button type="primary" @click="heneiwenjian">箱内文件</el-button></div>
                    </div>

                    <el-table ref="zaijielishitable" border height="450" id="ZJDAHdaochu"
                        @select-all="handleSelecthonghgl" :data="FileonloanTable"
                        @selection-change="handleSelectionChangehonghgl">
                        <el-table-column type="selection"></el-table-column>
                        <el-table-column show-overflow-tooltip :formatter="zaijieDAH" :prop="item.prop"
                            :label="item.label" width="100" v-for="(item, index) in zaijielishihgl"
                ...
                <el-tab-pane label="档案" name="second">
                    ...
                    <el-table ref="zaijielishitable_DA" border height="450" id="ZJDAdaochu"
                        @select-all="handleSelecthongdangan" :data="borrowingfilebox"
                        @selection-change="handleSelectionChangehonghgldangan">
                        <el-table-column type="selection"></el-table-column>
                        <el-table-column :formatter="zaijieDA" :show-overflow-tooltip='true' :prop="item.prop"
                            :label="item.label" width="100" v-for="(item, index) in zaijielishi"
                            :key="index"></el-table-column>
                    </el-table>
                </el-tab-pane>
            </el-tabs>
        ...
        <el-dialog :title="title" center :visible.sync="fileboxreturn">
            ...
            <div class="ModelInput">
                <div class="ModelInputone">
                    <div>
                        <label>归还人:</label>
                        <el-input style="width: 10vw; margin-left: 1vw; margin-right: 0.5vw"
                            v-model="inputgh"></el-input>
                        <span style="color: red">*</span>
                    </div>
                    <div>
                        <label>经办人:</label>

                        <el-input disabled style="width: 10vw; margin-left: 1vw; margin-right: 1vw"
                            v-model="inputjbr"></el-input>
                        <!--  -->
                    </div>
                </div>
        ...
        <el-dialog title="出库单" center :visible.sync="testshow" width="75%">
            <el-table :data="fileboxforloanList111" height="500" border><!--  -->
                <!-- <el-table-column type="selection" width="55" align="center"></el-table-column> -->
                <el-table-column type="expand" width="170" label="箱内档案信息" >
                    <template slot-scope="props">
                        <el-form label-position="left" inline>
                            <el-form-item label="档案信息:">
                        
                                    <div style="margin-left:1vw"  v-for="item in props.row.archiveInfo" >
                                    {{item }}
                                    </div>
                ...
                <el-table-column property="boxName" label="箱名"  align="center"></el-table-column>
                <el-table-column property="boxCode" label="箱编号"  align="center"></el-table-column>
                <el-table-column property="positionDes" label="位置信息"  align="center"></el-table-column>
                <!-- <el-table-column property="archiveInfo" label="档案信息" width="195" align="center"></el-table-column> -->
                ...
                <el-table-column property="status" label="状态" align="center">
                    <template slot-scope="{row}">
                        <el-button size="small" type="warning" icon="el-icon-check"
                            v-if="row.status == 0">未执行</el-button>
                        <el-button size="small" type="success" icon="el-icon-loading"
                            v-else-if="row.status == 1">{{zhixingstatus}}</el-button>
                        <el-button size="small" type="warning" icon="el-icon-loading"
                            v-else-if="row.status == 1">执行完成</el-button>
                        <!-- <el-button size="small" type="primary" icon="el-icon-takeaway-box" v-else>等待出库</el-button> -->
                    </template>
                </el-table-column>
                ...
                <el-table-column label="操作" align="center">
                    ...
                    <template slot-scope="scope">
                        <!-- <el-button size="mini" @click="chukuxiangnei(scope.$index, scope.row)">箱内信息</el-button> -->
                        <el-button type="success" size="mini" @click="zhixing(scope.$index, scope.row)">执行出库</el-button>
        ...
        <el-dialog title="出库历史" center :visible.sync="testshowhistory" width="75%">
            <el-table :data="fileboxforloanListtwo" height="500" border><!--  -->
                <!-- <el-table-column type="selection" width="55" align="center"></el-table-column> -->
                <el-table-column type="expand" width="170" label="箱内档案信息" >
                    <template slot-scope="props">
                        <el-form label-position="left" inline>
                ...
                <el-table-column property="status" label="状态" align="center">
                    <template slot-scope="{row}">
                        <!-- <el-button size="small" type="success" icon="el-icon-check"
                            v-if="row.status == 1">完成出库</el-button> -->
                        <el-button size="small" type="warning" icon="el-icon-loading"
                            v-if="row.status == 0">正在出库</el-button>
                        <!-- <el-button size="small" type="primary" icon="el-icon-takeaway-box" v-else>等待出库</el-button> -->
                    </template>
                </el-table-column>
            ...
            <el-menu :default-active="activeIndex2" class="el-menu-demo" mode="horizontal" @select="handleSelect"
                ...
                <!-- <el-submenu index="2">
                    ...
                    <el-submenu index="2-1">
                        ...
                        <el-menu-item index="2-1-2" style="color:#000" @click="Exceloutputchange()">档案箱</el-menu-item>
                    </el-submenu>
                    ...
                    <el-submenu index="2-4">
                        ...
                        <el-menu-item index="2-1-2" @click="DAMBHExceldownload()" style="color:#000">档案箱</el-menu-item>
                    </el-submenu>
                </el-submenu> -->
                <!-- <el-submenu style="background:#fff" index="3">

                    <template slot="title"><i class="el-icon-s-management"></i><span style="color:#000">
                            借阅管理</span></template>
                    <el-menu-item index="3-1" @click="atRorrowHistory()" style="color:#000">出箱历史</el-menu-item>
            ...
            <div style="color:#000" class="headerzhulu" @click="barCodePrint()">
              <i class="el-icon-s-fold"></i> 条码打印
                ...
                <el-menu-item index="6">
                    <div @click="chukubox()" class="headerzhulu">
                        <i class="el-icon-pie-chart"></i>出库单
                    </div>
                </el-menu-item>
                <el-menu-item index="6">
                    <div @click="chukuboxhistory()" class="headerzhulu">
                        <i class="el-icon-pie-chart"></i>出库历史
                    </div>
                </el-menu-item>
                <!-- 开架管理 -->
                <!-- <el-menu-item index="6">
        ...
        <div v-if="dapingshowone" class="containerone">
            ...
            <div style="display:flex;position: relative;">
                ...
                <div @mouseenter="shubiaoyiru()" @mouseleave="shubiaoyichu()" v-show="yidonghezi"
                    ...
                    <div style="width:4vw;height:4vh;">
                        <div style="display:flex;margin:1vw">
                            <el-button size="medium" type="success" @click="OnShelf()">出箱</el-button>
                            ...
                            <!-- <el-button size="mini" type="success" @click="fileboxforloanClick(1)">箱借阅</el-button> -->
                            ...
                            <el-dialog title="标签打印" :visible.sync="PrinterCodeDialog" width="50%" height="500" center>
                                <printer :IpPortPrinter_s="this.IpPortPrinter_s" @getValue="getValue"
                                    @flag="flagChange(arguments)" v-if="PrinterCodeDialog" />
                            </el-dialog>
                            <!-- YJJ -->
                        </div>
                ...
                <!-- <div v-show="isCollapse" :class="isCollapse ? 'newisCollapse' : ''">
            <div>
              <el-select v-model="valuetwo" placeholder="请选择" @change="xialaone1">
                <el-option v-for=" item  in  options " :key="item.value" :label="item.label" :value="item.value">
                </el-option>
              </el-select>
              <div>
                <el-tree style="background:#fff;color:#000;height:24vh;overflow-y: scroll;" :data="datatwo"
                  :props="defaultProps" @node-click="handleNodeClick"></el-tree>
              </div>
            </div>
            <div v-show="settinging" @click="setting()" style="border: 1px solid #000;width: 1.6vw;height: 1.6vw">
              <div class="shezhianniu"><img src="../../assets/设置 (4).png" alt=""></div>
                ...
                <!-- <el-table id="table-listone" ref="multipleTable" @row-click="btnDHA"
                    :class="newclass ? 'newclass' : 'oldclass'" height="400" :data="tableBodyInfoList"
                    ...
                    <el-table-column :width="item.width" v-for="( item, idx ) in  columnInfoList " :key="idx"
                        :prop="item.prop" :show-overflow-tooltip="true" :label="item.label" header-align="center"
                        align="center">
                    </el-table-column>
                        ...
                        <template slot-scope="scope">
                            <div style="display:flex;">
                                ...
                                <el-button size="mini" type="danger"
                                    @click="handleDelete(scope.$index, scope.row)">回收</el-button>
                            </div>
                        </template>
                    </el-table-column>
                </el-table> -->
                <el-table ref="xyese" :data="tableBodyInfoList" height="285" border
                    :class="newclass ? 'newclass' : 'oldclass'" @selection-change="handleSelectionbox">
                    ...
                    <el-table-column label="操作" align="center">
                        ...
                        <template slot-scope="scope">
                            <el-button type="success" size="mini" @click="xiangneishow(scope.$index, scope.row)">箱内信息</el-button>
            ...
            <div style="margin-top: 0.3vw;float: right">
                <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
                    :current-page="pIndex_dah" :page-sizes="[10, 20, 50, 100]" :page-size=pSize_dah
                    layout="total, sizes, prev, pager, next, jumper" :total="count_DAH">
                </el-pagination>
            </div>
        ...
        <div v-show="dapingshow" class="containertwo">
            ...
            <div style="display:flex;position: relative;">
                <!-- 档案菜单下拉部分 -->
                <div style="position: absolute;left:0;top:-0.5vh;background:#e0e4ec; border: 1px solid #6792f0;z-index:9999999999;width:40vw;height:8vh;"
                    @mouseenter="shubiaoyiruone()" @mouseleave="shubiaoyichuone()" v-show="yidonghezione">
                    <div style="width:4vw;height:4vh;">
                        <div style="display:flex;margin:1vw">
                            <!-- <el-button size="mini" type="success" @click="DA_fileboxforloanClick(1)">装箱</el-button> -->
                            <el-button size="mini" type="warning" @click="generat()">拆箱</el-button>
          ...
          <div @click="collapseone" :class="isCollapseone ? 'zhediantwo' : 'zhedianonetwo'"></div> -->
                ...
                <el-table @row-click="btnDA" id="table-listoneDA" :class="newclassone ? 'newclassone' : 'oldclassone'"
                    height="400" @selection-change="handleSelectionChange" :data="DAtableBodyInfoList" border
                    ref="multipleTableone" style="width: 100%;overflow-x:visible; white-space: nowrap;">
                    <!-- yanjiajie -->

                    <el-table-column type="selection" header-align="center" align="center">
                    </el-table-column>
                    <el-table-column v-if="this.DAtableBodyInfoList.length != 0" label="序号" align="center">
                        <template slot-scope="scope">
                            {{ scope.$index + 1 }}
                        </template>
                    </el-table-column>
                    <el-table-column :show-overflow-tooltip="true" :width="item.width"
                        v-for="( item, idx ) in  DAcolumnInfoList " :key="idx" :prop="item.prop" :label="item.label"
                        header-align="center" align="center">
                    </el-table-column>
                    <!-- yanjiajie -->
                ...
                <el-button size="mini" type="success" v-if="DAtableBodyInfoList[scope.$index].status == '未装盒'"
                  @click="DA_OnBox(scope.$index, scope.row)">装盒</el-button> -->
                    <!-- <el-button size="mini" type="danger"
                                @click="handleDeleteS(scope.$index, scope.row)">回收</el-button>
                        </template>
                    </el-table-column> -->
                </el-table>
            </div>
            <div style="float: right;">
                <el-pagination @size-change="handleSizeChangeone" @current-change="handleCurrentChangeone"
                    :current-page="pIndex_da" :page-sizes="[5, 20, 50, 100]" :page-size="pSize_da"
            ...
            <el-form>
                    ...
                    <el-descriptions title="搜索条件" border>
                        ...
                        <template slot="extra">
                            <el-button type="primary" icon="el-icon-refresh-right" circle
                                @click="reset()"></el-button>复位
                            <el-button type="success" icon="el-icon-folder-add" circle @click="add()"></el-button>添加搜索条件
                            <el-button type="danger" icon="el-icon-folder-remove" circle
                                @click="remove()"></el-button>删除搜索条件
                        </template>
                        ...
                        <template v-for="(item, index) in RecordInputListearch">
                            <el-descriptions-item :key="index">
                                ...
                                <el-select v-model="item.sel" style="width: 10vw;margin-left: auto;"
                                    placeholder="请选择著录项">

                                    <el-option v-for="(item, index) in DAoptions" :label="item.label" :key="index"
                                        :value="item.label"></el-option>
                                </el-select>
                                <el-select v-model="item.chse" style="width: 10vw;margin-left: auto;"
                                    placeholder="请选择判断条件">
                ...
                <el-form-item>
                    <el-descriptions title="排序方式" border>
                        ...
                        <template slot="extra">
                            <el-button type="primary" icon="el-icon-refresh-right" circle
                                @click="resetpaixu()"></el-button>复位
                            <el-button type="success" icon="el-icon-folder-add" circle
                                @click="addpaixu()"></el-button>添加搜索条件
                            <el-button type="danger" icon="el-icon-folder-remove" circle
                                @click="removepaixu()"></el-button>删除排序条件
                        </template>
        ...
        <el-dialog :append-to-body="true" center title="统计" :visible.sync="tongjishow" width="60%">
            <div style="height: 30vh;">
                ...
                <div>
                    <el-descriptions title="档案箱" :column="3" border>
                        <el-descriptions-item label="总数量" label-class-name="my-label">{{ DAH_con
                            }}</el-descriptions-item>
                        <el-descriptions-item label="已上架" label-class-name="my-label">{{ DAH_yishngjia
                            }}</el-descriptions-item>
                        <el-descriptions-item label="未上架" label-class-name="my-label">{{ DAH_weishngjia
                            }}</el-descriptions-item>
                        <el-descriptions-item label="待归还" label-class-name="my-label">{{ DAH_daiguihuan
                            }}</el-descriptions-item>
                ...
                <div>
                    <el-descriptions title="档案" :column="3" border>
                        ...
                        <el-descriptions-item label="已上架" label-class-name="my-label">{{ yishngjia
                            }}</el-descriptions-item>
                        <el-descriptions-item label="未上架" label-class-name="my-label">{{ weishngjia
                            }}</el-descriptions-item>
                        <el-descriptions-item label="待归还" label-class-name="my-label">{{ daiguihuan
                            }}</el-descriptions-item>
        ...
        <el-dialog center title="盒内数据" :visible.sync="heneishuju" width="80%">
            <div style="height: 55vh;">
                ...
                <div style="margin-top:2vw">
                    <el-table id="table-listoneDA" height="500" @selection-change="handleSelectionChange"
                        ...
                        <el-table-column :show-overflow-tooltip="true" v-for="( item, idx ) in  kuaisujieyueheinei "
            ...
            <!-- <span slot="footer" class="dialog-footer">
          <el-button @click="heneishuju = false">取 消</el-button>
          <el-button type="primary" @click="heneishuju = false">确 定</el-button>
        </span> -->
        </el-dialog>
...
export default {
    ...
    data() {
        return {
            //档案盒归还
            fileboxforloanList111: [
                { ID: 1, Info: '测试档案箱1', Number: "CX5-WS.WJ-1999-0001", CODE: "CCCCCC20240221192059", Address: '综合档案库一区2列左侧2节一层', status: 1, },
                { ID: 2, Info: '测试档案箱2', Number: "CX5-WS.WJ-1999-0002", CODE: "CCCCCC20240222192053", Address: '综合档案库一区2列左侧2节二层', status: 1, },
                { ID: 3, Info: '测试档案箱3', Number: "CX5-WS.WJ-1999-0003", CODE: "CCCCCC20240223192058", Address: '综合档案库一区2列左侧2节三层', status: 0, },
                { ID: 4, Info: '测试档案箱4', Number: "CX5-WS.WJ-1999-0004", CODE: "CCCCCC20240224192053", Address: '综合档案库一区2列左侧3节一层', status: 0, },
                { ID: 5, Info: '测试档案箱5', Number: "CX5-WS.WJ-1999-0005", CODE: "CCCCCC20240225192051", Address: '综合档案库一区2列左侧3节二层', status: 0, },
                { ID: 6, Info: '测试档案箱6', Number: "CX5-WS.WJ-1999-0006", CODE: "CCCCCC20240226192054", Address: '综合档案库一区2列左侧3节三层', status: 0, },
            ],
          zhixingstatus:'执行中',
            chukuxiangshow: false,
            chaixianglist: [],//拆
            chaixiang: {},
            testshow: false,
            testshowhistory:false,
            PortPrinterServe: '',
            IpPrinterServe: '',
            PortPrinter: '',
            IpPrinter: '',
    ...
    mounted() {
        this.fileboxforloanList22222 = []
        bus.$off('changeEventttt');
        bus.$on('changeEventttt', (MLId, MLName, FatherKey, MLGS, danganjieyueflag) => {
            console.log('进来一次');
            console.log(MLId, MLName, FatherKey, MLGS, '消息通信2222222222222');
    ...
    methods: {
        chukuxiangnei() {
            this.chukuxiangshow = true
        },
        zhixing(index,item) {
// console.log(item,'3333333333333');
// axios.get('http://' + this.ServeIp + ':' + this.ServePort + '/outboundOrder/execute?id='+item.orderId).then(res => {
//                 if (res.data.code == 200) {
                
//                    this.zhixingstatus=res.data.table[0].states
...
//             })
        ...
        chuku() {
            ...
            axios.post('http://' + this.ServeIp + ':' + this.ServePort + '/outboundOrder/saveByArchive', arr).then(
                success => {
                    if (success.data.code == 200) { }
                    this.$message.success('成功进入出库单')
        ...
        getDATableList(valindex, valsize) {
            ...
            var data = {
                tableName: this.DAH_TableDesignTittle,
                pageNumber: valindex,
                pageSize: valsize,

            };
            axios.post("http://" + this.ServeIp + ":" + this.ServePort + "/boxManage/queryArchiveInfoInBox", data).then(
                (res) => {
        ...
        chukubox() {
            ...
            this.fileboxforloanList111 = []
            axios.post('http://' + this.ServeIp + ':' + this.ServePort + '/outboundOrder/query', arr).then(
                success => {
                    success.data.records.forEach((item,index)=>{
                        var archiveInfo= item.archiveInfo.split('_');   
                        this.fileboxforloanList111.push(
                            {
        ...
        chukuboxhistory() {
            ...
            this.fileboxforloanListtwo = []
            axios.post('http://' + this.ServeIp + ':' + this.ServePort + '/outboundOrder/query', arr).then(
                success => {
                    success.data.records.forEach((item,index)=>{
                        var archiveInfo= item.archiveInfo.split('_');   
                        this.fileboxforloanListtwo.push(
                            {
                                "orderId":item.orderId,
                                "archiveInfo":archiveInfo,
                                "boxCode":item.boxCode,
        ...
        //箱内信息
        xiangneishow(index, item) {
            this.DAtableBodyInfoList = [];
            this.DAcolumnInfoList = [];
            this.count_DA = 0
            console.log(item.dacode, '322222222222222222222222221111111111');
            let code = item.dacode
            axios.get('http://' + this.ServeIp + ':' + this.ServePort + '/boxManage/getArchiveCountInfoByBoxCode?boxCode=' + code).then(
                success => {
                    if (success.data.code == 200) {
                        this.$message.success(success.data.message)

                    }
                    console.log(success, '959555555555555555555555');
                    // bus.$off('xiangneixin111');
                    bus.$emit('xiangneixin111', success)
                },
                error => {
                    // console.log(error, '开架管理--返回失败的数据');
                })
        ...
        export2Excel() {
            ...
            require.ensure([], () => {
                ...
                export_json_to_excel(tHeader, data, '档案箱选中数据')// 导出的表格名称，可自定义
            })
        },
        //档案盒格式转换
        ...
        OnShelf() {//档案盒 上架
            this.shangjiaflag = 1
            if (this.DAHlist.length < 1) {
                this.$message.error('请选择要上架的档案箱数据')
                return
            }
        ...
        DownShelf() {//档案盒 下架
            if (this.DAHlist.length < 1) {
                this.$message.error('请选择要下架的档案箱数据')
                return
            }
            this.xiajiaflag = 1
        ...
        handleSelectionChange(val) {
            ...
            this.chaixianglist = []
            this.multiSelectallboxesList.forEach((item) => {

                this.chaixianglist.push({
                    "archiveTableName": this.DAH_TableDesignTittle,
                    "archiveId": item.id
                })
                console.log(this.chaixianglist, 'this.chaixianglistthis.chaixianglist');
            })
        },
        //
        ...
        DAMBHExceldownload() {
            ...
            let fileName = this.Bus_MLName + "_" + this.Bus_MLId + "_档案箱_Excel模板.xls";
